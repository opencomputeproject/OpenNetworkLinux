...
static ssize_t show_psu_eeprom(struct device *dev, 
                               ...
                               char *buf);
static struct nba750_psu_data *nba750_psu_update_status(struct device *dev);
static struct nba750_psu_data *nba750_psu_update_eeprom(struct device *dev);
static int nba750_psu_read_block(struct i2c_client *client, 
                                u8 command, 
                                u8 *data,
                                int data_len);
...
show_psu_pg(struct device *dev, 
            ...
            char *buf)
{
    struct nba750_psu_data *data = nba750_psu_update_status(dev);
    unsigned int value;
...
static struct attribute *nba750_psu_attributes[] = {
...
show_psu_eeprom(struct device *dev, 
                ...
                char *buf)
{
    struct nba750_psu_data *data = nba750_psu_update_eeprom(dev);
    
    memcpy(buf, (char *)data->eeprom, EEPROM_SZ);
    return EEPROM_SZ;
}

static const struct attribute_group nba750_psu_group = {
    .attrs = nba750_psu_attributes,
};
...
static int 
nba750_psu_probe(struct i2c_client *client,
                const struct i2c_device_id *dev_id)
{
    struct nba750_psu_data *data;
    int status, err;
    ...
    data = kzalloc(sizeof(struct nba750_psu_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    memset(data, 0, sizeof(struct nba750_psu_data));
    ...
    status = sysfs_create_group(&client->dev.kobj, &nba750_psu_group);
    if (status) {
        goto exit_free;
    }
...
{
    struct nba750_psu_data *data = i2c_get_clientdata(client);

    sysfs_remove_group(&client->dev.kobj, &nba750_psu_group);
    kfree(data);
 ...
 */
static int 
nba750_psu_read_block(struct i2c_client *client, 
                     u8 command, 
                     u8 *data,
                     int data_len)
{
 ...
 */
static struct nba750_psu_data 
*nba750_psu_update_eeprom(struct device *dev)
{
    ...
    if (time_after(jiffies, data->last_updated + 300 * HZ)
        ...
        if (!data->psuABS) {
            ...
            //read eeprom
            status = nba750_psu_read_block(client, 0, data->eeprom, 
 ...
 */
static struct nba750_psu_data 
*nba750_psu_update_status(struct device *dev)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct nba750_psu_data *data = i2c_get_clientdata(client);
    s32 status = 0;
...
static const struct i2c_device_id nba750_psu_id[] = {
    { "psu1", nba750_psu1 },
    { "psu2", nba750_psu2 },
    {}
};

MODULE_DEVICE_TABLE(i2c, nba750_psu_id);

static struct i2c_driver nba750_psu_driver = {
    .driver = {
        .name     = DRIVER_NAME,
    },
...
static int __init nba750_psu_init(void)
{
    return i2c_add_driver(&nba750_psu_driver);
}

static void __exit nba750_psu_exit(void)
{
    i2c_del_driver(&nba750_psu_driver);
}

module_init(nba750_psu_init);
module_exit(nba750_psu_exit);

MODULE_AUTHOR("Netberg <support@netbergtw.com>");
MODULE_DESCRIPTION("Aurora 750 psu driver");
MODULE_LICENSE("GPL");
