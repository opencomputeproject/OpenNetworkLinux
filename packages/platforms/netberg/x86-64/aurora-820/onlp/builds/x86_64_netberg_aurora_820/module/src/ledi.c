...
static int _sys_onlp_ledi_info_get(onlp_oid_t id, onlp_led_info_t* info)
{
    ...
    *info= __onlp_led_info[led_id];
    ...
    rv = onlp_file_read((uint8_t*)buf,ONLP_CONFIG_INFO_STR_MAX, &len,NET_LED_PREFIX"grn_led");
    if(rv != ONLP_STATUS_OK) {
        return rv;
    }
    grn_mode=(int)(buf[0]-'0');

    rv = onlp_file_read((uint8_t*)buf,ONLP_CONFIG_INFO_STR_MAX, &len,NET_LED_PREFIX"red_led");
    if(rv != ONLP_STATUS_OK) {
        return rv;
    }
...
static int _fan_onlp_ledi_info_get(onlp_oid_t id, onlp_led_info_t* info)
{
    ...
    *info=__onlp_led_info[led_id];
    ...
    if( info->status & ONLP_LED_STATUS_PRESENT) {
        rv = onlp_file_read((uint8_t*)buf,ONLP_CONFIG_INFO_STR_MAX, &len, NET_LED_PREFIX"fanmodule%d_led", fan_id);
        if(rv == ONLP_STATUS_OK ) {
            switch(buf[0]) {
            case '0':
...
static int _sys_onlp_ledi_mode_set(onlp_led_mode_t onlp_mode)
{
    ...
    if( onlp_mode == ONLP_LED_MODE_OFF) {
        rv = onlp_file_write_int(SYS_LED_MODE_OFF, NET_LED_PREFIX"grn_led");
        if(rv != ONLP_STATUS_OK ) {
            return rv;
        }
        rv = onlp_file_write_int(SYS_LED_MODE_OFF, NET_LED_PREFIX"red_led");
    } else {
...
int onlp_ledi_status_get(onlp_oid_t id, uint32_t* rv)
{
    ...
    if(result == ONLP_STATUS_OK) {
        ...
        switch(led_id) {
        case ONLP_LED_MGMT:
            result = onlp_file_read_int((int*)&grn_mode, NET_LED_PREFIX"grn_led");
            if(result != ONLP_STATUS_OK) {
                return result;
            }
            result = onlp_file_read_int((int*)&red_mode, NET_LED_PREFIX"red_led");
            if(result != ONLP_STATUS_OK) {
                return result;
            }
        ...
        case ONLP_LED_FAN6:
            ...
            if(fan_status & ONLP_FAN_STATUS_PRESENT) {
                ...
                result = onlp_file_read_int((int*)&mode, NET_LED_PREFIX"fanmodule%d_led", fan_id);
                if(result != ONLP_STATUS_OK) {
                    return result;
                }
