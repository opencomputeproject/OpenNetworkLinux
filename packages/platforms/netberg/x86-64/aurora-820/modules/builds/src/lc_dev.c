...
#define net_set_bit(bit, reg)  ((reg) | (1 << bit))
#define net_clear_bit(bit, reg)  ((reg) & (~(1 << bit)))
...
static int lc_sff_output_set(lc_sff_output_type_t type, int lc_id, int port, u8 val)
{
    ...
    if (val) {
        reg = net_set_bit(idx, reg);
    } else {
        reg = net_clear_bit(idx, reg);
    }
    /*debug*/
...
int lc_dev_reset_set(int lc_id, u8 lv)
{
    ...
    LC_DEV_LOG_DBG("lc_id:%d get reg:0x%x\n", lc_id, reg);
    if (lv) {
        reg = net_set_bit(SWICH_CPLD_LC_CPU_PLTRST_N_BIT, reg);
    } else {
        reg = net_clear_bit(SWICH_CPLD_LC_CPU_PLTRST_N_BIT, reg);
    }
    LC_DEV_LOG_DBG("lc_id:%d set reg:0x%x\n", lc_id, reg);
...
int lc_dev_12v_set(int lc_id, bool on)
{
    ...
    LC_DEV_LOG_DBG("lc_id:%d get reg:0x%x\n", lc_id, reg);
    if (on) {
        reg = net_set_bit(SWICH_CPLD_LC_PWR_CONTROL_BIT, reg);
    } else {
        reg = net_clear_bit(SWICH_CPLD_LC_PWR_CONTROL_BIT, reg);
    }
    LC_DEV_LOG_DBG("lc_id:%d set reg:0x%x\n", lc_id, reg);
