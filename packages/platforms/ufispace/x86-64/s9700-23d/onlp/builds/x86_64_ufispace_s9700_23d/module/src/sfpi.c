...
onlp_sfpi_is_present(int port)
{
    ...
    //QSFPDD_NIF, QSFPDD_FAB Ports
    if (port < (QSFPDD_NIF_NUM + QSFPDD_FAB_NUM)) { //QSFPDD
        if (qsfp_present_get(port, &status) != ONLP_STATUS_OK) {
            AIM_LOG_ERROR("qsfp_presnet_get() failed, port=%d\n", port);
            return ONLP_STATUS_E_INTERNAL;
        }
    } else if (port >= (QSFPDD_NIF_NUM + QSFPDD_FAB_NUM) && port < PORT_NUM) { //SFP
...
{   
    ...
    int size = 0;
    int ret, sfp_port_num;

    memset(data, 0, 256);
    memset(eeprom_path, 0, sizeof(eeprom_path));

    //QSFPDD_NIF, QSFPDD_FAB, SFP Ports
    if (port >= 0 && port < QSFPDD_NIF_NUM) { //QSFPDD_NIF
        snprintf(eeprom_path, sizeof(eeprom_path), "/sys/bus/i2c/devices/%d-0050/eeprom", port+25);
    } else if (port >= QSFPDD_NIF_NUM && port < (QSFPDD_NIF_NUM + QSFPDD_FAB_NUM)) { //QSFPDD_FAB
        snprintf(eeprom_path, sizeof(eeprom_path), "/sys/bus/i2c/devices/%d-0050/eeprom", port+31);
    } else if (port >= (QSFPDD_NIF_NUM + QSFPDD_FAB_NUM) && port < PORT_NUM) { //SFP
        sfp_port_num = port - QSFPDD_NIF_NUM - QSFPDD_FAB_NUM;
        if (sfp_port_num == 0) {
...
onlp_sfpi_rx_los_bitmap_get(onlp_sfp_bitmap_t* dst)
{
    ...
    for(i = 0; i < (QSFPDD_NIF_NUM + QSFPDD_FAB_NUM); i++) {
        AIM_BITMAP_MOD(dst, i, 0);
    }

    /* Populate bitmap - SFP+ ports */
    for(i = (QSFPDD_NIF_NUM + QSFPDD_FAB_NUM); i < PORT_NUM; i++) {
...
onlp_sfpi_control_get(int port, onlp_sfp_control_t control, int* value)
{
    ...
    char sysfs_path[128];
    int cpld_addr = 0x30;
    int qsfpdd_num = (QSFPDD_NIF_NUM + QSFPDD_FAB_NUM);
