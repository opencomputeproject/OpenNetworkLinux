...
static ssize_t write_reset_mac(struct device *dev,
        struct device_attribute *da, const char *buf, size_t count);
static ssize_t read_reset_retimer(struct device *dev,
                struct device_attribute *da, char *buf);
...
static struct attribute *apollo_cpld1_attributes[] = {
    ...
    &sensor_dev_attr_cpld_qsfp_port_status_0.dev_attr.attr,
...
/* cpld 2 / cpld 3 */
static struct attribute *apollo_cpld23_attributes[] = {
    &sensor_dev_attr_cpld_access_register.dev_attr.attr,
    &sensor_dev_attr_cpld_register_value.dev_attr.attr,
    &sensor_dev_attr_cpld_qsfp_port_start.dev_attr.attr,
    &sensor_dev_attr_cpld_qsfp_ports.dev_attr.attr,
...
/* cpld 5 */
static struct attribute *apollo_cpld5_attributes[] = {
    &sensor_dev_attr_cpld_access_register.dev_attr.attr,
    &sensor_dev_attr_cpld_register_value.dev_attr.attr,
    &sensor_dev_attr_cpld_qsfpdd_port_start.dev_attr.attr,
    &sensor_dev_attr_cpld_qsfpdd_ports.dev_attr.attr,
...
static const struct attribute_group apollo_cpld23_group = {
    .attrs = apollo_cpld23_attributes,
};
...
static const struct attribute_group apollo_cpld5_group = {
                    ...
                    char *buf)
{
    ...
    if (attr->index >= CPLD_QSFP_PORT_STATUS_0 &&
        attr->index <= CPLD_QSFP_PORT_STATUS_12) {
        reg = CPLD_QSFP_PORT_STATUS_BASE_REG +
            (attr->index - CPLD_QSFP_PORT_STATUS_0);
        I2C_READ_BYTE_DATA(reg_val, &data->access_lock, client, reg);
        if (reg_val < 0)
            return -1;
        return sprintf(buf, "0x%02x\n", reg_val);
                    ...
                    char *buf)
{
    ...
    if (attr->index >= CPLD_QSFP_PORT_CONFIG_0 &&
        attr->index <= CPLD_QSFP_PORT_CONFIG_12) {
                    ...
                    char *buf)
{
    ...
    if (attr->index >= CPLD_QSFPDD_PORT_STATUS_0 &&
        attr->index <= CPLD_QSFPDD_PORT_STATUS_10) {
	if (data->index == cpld4) {
	    reg_base = CPLD4_QSFPDD_PORT_STATUS_BASE_REG;
	} else {
                    ...
                    char *buf)
{
    ...
    if (attr->index >= CPLD_QSFPDD_PORT_CONFIG_0 &&
        attr->index <= CPLD_QSFPDD_PORT_CONFIG_10) {
	if (data->index == cpld4) {
	    reg_base = CPLD4_QSFPDD_PORT_CONFIG_BASE_REG;
	} else {
...
/* get reset retimer register value */
static ssize_t read_reset_retimer(struct device *dev,
                    struct device_attribute *da,
                    char *buf)
{
    ...
    if (attr->index == CPLD_RESET_RETIMER) {
        reg = CPLD_RESET_RETIMER_REG;
        I2C_READ_BYTE_DATA(reg_val, &data->access_lock, client, reg);
        if (reg_val < 0)
            return -1;
        return sprintf(buf, "0x%02x\n", reg_val);
...
/* set value to reset retimer register */
static ssize_t write_reset_retimer(struct device *dev,
                    struct device_attribute *da,
                    const char *buf,
                    size_t count)
...
exit:
    ...
    case cpld4:
        ...
        break;
    case cpld5:
        sysfs_remove_group(&client->dev.kobj, &apollo_cpld5_group);
        break;
    default:
        break;
    }
