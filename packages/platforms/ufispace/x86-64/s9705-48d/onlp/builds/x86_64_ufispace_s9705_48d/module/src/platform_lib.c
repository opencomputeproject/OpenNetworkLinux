...
psu_present_get(int *pw_present, int id)
{
    ...
    if ((rc = file_read_hex(&status, "/sys/bus/i2c/devices/%d-0030/cpld_psu_status_%d",
                                 I2C_BUS_2, psu_index)) != ONLP_STATUS_OK) {
        return ONLP_STATUS_E_INTERNAL;
    }
...
sysi_platform_info_get(onlp_platform_info_t* pi)
{
    ...
    char ucd_out_top[48], ucd_out_bot[48];
    char ucd_ver_top[8], ucd_ver_bot[8];
    char ucd_date_top[8], ucd_date_bot[8];
    ...
    memset(ucd_out_top, 0, sizeof(ucd_out_top));
    memset(ucd_out_bot, 0, sizeof(ucd_out_bot));
    memset(ucd_ver_top, 0, sizeof(ucd_ver_top));
    memset(ucd_ver_bot, 0, sizeof(ucd_ver_bot));
    memset(ucd_date_top, 0, sizeof(ucd_date_top));
    memset(ucd_date_bot, 0, sizeof(ucd_date_bot));
    ...
    for(i=0; i<CPLD_MAX; ++i) {
        //cpld_ver[i] = onlp_i2c_readb(I2C_BUS_2, CPLD_BASE_ADDR + i, CPLD_REG_VER, ONLP_I2C_F_FORCE);
        if ((rc = file_read_hex(&cpld_ver[i], "/sys/bus/i2c/devices/2-00%02x/cpld_version",
    ...
    //get UCD version
    if (exec_cmd(CMD_UCD_VER_TOP, ucd_out_top, sizeof(ucd_out_top)) < 0 ||
        exec_cmd(CMD_UCD_VER_BOT, ucd_out_bot, sizeof(ucd_out_bot)) < 0) { 
            AIM_LOG_ERROR("unable to read UCD version\n");
            return ONLP_STATUS_E_INTERNAL; 
    }

    //parse UCD version and date
    ucd_len = get_ipmitool_len(ucd_out_top);
    if (ucd_len > ucd_date_len) {
        parse_ucd_out(ucd_out_top, ucd_ver_top, 0, ucd_len-ucd_date_len);
        parse_ucd_out(ucd_out_top, ucd_date_top, ucd_len-ucd_date_len, ucd_date_len);
    } else {
        parse_ucd_out(ucd_out_top, ucd_ver_top, 0, ucd_len);
    }
    ucd_len = get_ipmitool_len(ucd_out_bot);
    if (ucd_len > ucd_date_len) {
        parse_ucd_out(ucd_out_bot, ucd_ver_bot, 0, ucd_len-ucd_date_len);
        parse_ucd_out(ucd_out_bot, ucd_date_bot, ucd_len-ucd_date_len, ucd_date_len);
    } else {
        parse_ucd_out(ucd_out_bot, ucd_ver_bot, 0, ucd_len);
    }
    ...
    pi->other_versions = aim_fstrdup(
        ...
        ucd_ver_top, ucd_date_top,
        ucd_ver_bot, ucd_date_bot);
...
bmc_fan_info_get(onlp_fan_info_t* info, int id)
{
    ...
    if (id >= FAN_ID_FAN1 && id <= FAN_ID_FAN4) {
        rv = bmc_sensor_read(id + 14, FAN_SENSOR, &data);
    ...
    rv = bmc_sensor_read(id + 6, FAN_SENSOR, &data);
    if ( rv != ONLP_STATUS_OK) {
        AIM_LOG_ERROR("unable to read sensor info from BMC, sensor=%d\n", id);
        return rv;
    }
...
sys_led_info_get(onlp_led_info_t* info, int id)
{
    ...
    if ((rc = file_read_hex(&value, "/sys/bus/i2c/devices/%d-0030/cpld_system_led_%d",
                                 I2C_BUS_2, sysfs_index)) != ONLP_STATUS_OK) {
        return ONLP_STATUS_E_INTERNAL;
    }
