...
{
    return "x86-64-delta-agc5648s-r0";
}
...
int onlp_sysi_platform_info_get(onlp_platform_info_t* pi)
{
    ...
    rv = ifnOS_LINUX_BmcI2CGet(I2C_BMC_BUS_3, SWPLD_1_ADDR, CPLD_VERSION_REGISTER, &u4Data, DATA_LEN);
...
{ 
    ...
    for(i = 0; i < NUM_OF_FAN_ON_MAIN_BROAD; i++)
    {
        ...
        {
            fan_tray_new_mode[i] = ONLP_LED_MODE_OFF;
            continue;
        }
        else
        {
            ...
            {
                fan_tray_new_mode[i] = ONLP_LED_MODE_RED;
                continue;
            }
        }
    ...
    onlp_ledi_mode_set(ONLP_LED_ID_CREATE(LED_FAN_TRAY_5), fan_tray_new_mode[4]);
    ...
    if((fan_tray_new_mode[0] == ONLP_LED_MODE_GREEN) && (fan_tray_new_mode[1] == ONLP_LED_MODE_GREEN) && 
       ...
       (fan_tray_new_mode[4] == ONLP_LED_MODE_GREEN) )
    {
        fan_new_mode = ONLP_LED_MODE_GREEN;
    }
    else if((fan_tray_new_mode[0] == ONLP_LED_MODE_OFF) || (fan_tray_new_mode[1] == ONLP_LED_MODE_OFF) || 
            (fan_tray_new_mode[2] == ONLP_LED_MODE_OFF) || (fan_tray_new_mode[3] == ONLP_LED_MODE_OFF) ||
            (fan_tray_new_mode[4] == ONLP_LED_MODE_OFF) )
    {
    ...
    if((psu_info[0].status & ONLP_PSU_STATUS_PRESENT) && (psu_info[1].status & ONLP_PSU_STATUS_PRESENT))
    {
        if(!(psu_info[0].status & ONLP_PSU_STATUS_FAILED) && !(psu_info[1].status & ONLP_PSU_STATUS_FAILED))
        {
            psu_new_mode = ONLP_LED_MODE_GREEN;
        }
        else if(!(psu_info[0].status & ONLP_PSU_STATUS_FAILED) || !(psu_info[1].status & ONLP_PSU_STATUS_FAILED))
        {
    ...
    else if((psu_info[0].status & ONLP_PSU_STATUS_PRESENT) || (psu_info[1].status & ONLP_PSU_STATUS_PRESENT))
    {
        if(!(psu_info[0].status & ONLP_PSU_STATUS_FAILED) || !(psu_info[1].status & ONLP_PSU_STATUS_FAILED))
        {
            psu_new_mode = ONLP_LED_MODE_YELLOW;
        }
        else
        {
            psu_new_mode = ONLP_LED_MODE_OFF;
