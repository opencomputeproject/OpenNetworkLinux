...
static int dni_psu_info_get(onlp_oid_t id, onlp_psu_info_t* info)
{
    ...
    info->hdr.coids[1] = ONLP_THERMAL_ID_CREATE(local_id*2 - 1 + CHASSIS_THERMAL_COUNT);
    info->hdr.coids[2] = ONLP_THERMAL_ID_CREATE(local_id*2 + CHASSIS_THERMAL_COUNT);
    ...
    /* Get psu Voltage_1/Voltage_2 */
    sprintf(device_name, "PSU%d_Voltage_%d", local_id, local_id);
...
int onlp_psui_info_get(onlp_oid_t id, onlp_psu_info_t* info)
{
    ...
    *info = psu_info[index];
    ...
    sprintf(device_name, "PSU%d_Voltage_%d", index, index);
    while (strcmp(psu_dev_list[dev_list_index].dev_name, device_name) != 0)
        dev_list_index++;
    if (dni_bmc_sensor_read(psu_dev_list[dev_list_index].dev_name,
                            &u4Data,
