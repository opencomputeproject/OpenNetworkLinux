...
dni_psu_pmbus_info_get(int id, char *node, int *value)
{
    ...
    *value = 0;
    ...
    switch (id) {
        case PSU1_ID:
            sprintf(node_path, "%s%s", PSU_AC_PMBUS_PREFIX, node);
            break;
        default:
            break;
    }
...
dni_psu_info_get(onlp_psu_info_t* info)
{
    ...
    char val_char_mod[12] = {'\0'};
    char val_char_sel[12] = {'\0'};
    char node_path[PSU_NODE_MAX_PATH_LEN] = {'\0'};
    ...
    sprintf(node_path, "%s%s", PSU_AC_PMBUS_PREFIX, "psu_mfr_model");
    ...
    sprintf(node_path, "%s%s", PSU_AC_PMBUS_PREFIX, "psu_mfr_serial");
