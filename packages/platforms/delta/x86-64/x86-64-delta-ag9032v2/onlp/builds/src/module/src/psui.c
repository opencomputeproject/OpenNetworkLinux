...
/*
    UINT4 byte_get   = 0;
    UINT4 low_bit, high_bit;

    / Read front LED (PSU1/PSU2)is present or not /
    byte_get = onlp_i2c_readb(I2C_BUS_1, SWPLD_1_ADDR, SYS_LED1_REGISTER, ONLP_I2C_F_TENBIT);
    
    if (local_id == PSU1_ID){
        byte_get = (byte_get >> 6) & (3);
    }
    else if (local_id == PSU2_ID){
        byte_get = (byte_get >> 4) & (3);
    }
    low_bit = byte_get & 1;
    high_bit = byte_get >> 1;
*/
    ...
    switch (local_id) {
        ...
        case PSU2_ID:
            ...
            if ( dni_psui_eeprom_info_get(module_name, local_id, PSU_MODEL_REG) == ONLP_STATUS_E_INVALID ){ //high_bit == low_bit &&
                info->status = ONLP_PSU_STATUS_FAILED;
                break;
            }
            //get psu Pin/Pout
            sprintf(device_name, "PSU%d_Pin",local_id);
            if(dni_sensor_get(device_name, &u4Data, multiplier) == 0){
                info->mpin   = u4Data ;
