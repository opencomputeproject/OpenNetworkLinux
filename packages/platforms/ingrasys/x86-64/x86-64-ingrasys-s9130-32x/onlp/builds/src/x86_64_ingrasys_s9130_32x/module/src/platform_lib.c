...
{     
    ...
    val = onlp_i2c_readb(I2C_BUS_17, reg_addr, offset, ONLP_I2C_F_FORCE);
    if (val < 0) {
        return ONLP_STATUS_E_INTERNAL; 
    }
...
system_led_set(onlp_led_mode_t mode)
{
    ...
    if(mode == ONLP_LED_MODE_GREEN) {        
        rc = onlp_i2c_modifyb(I2C_BUS_2, LED_REG, LED_OFFSET, LED_SYS_AND_MASK,
...
fan_led_set(onlp_led_mode_t mode)
{
    int rc;
    if(mode == ONLP_LED_MODE_GREEN) {
        rc = onlp_i2c_modifyb(I2C_BUS_2, LED_REG, LED_OFFSET, LED_FAN_AND_MASK,
...
psu1_led_set(onlp_led_mode_t mode)
{
    int rc;
    if(mode == ONLP_LED_MODE_GREEN) {    
        rc = onlp_i2c_modifyb(I2C_BUS_2, LED_REG, LED_OFFSET, 
...
memset(cmd, 0, sizeof(cmd));
    ...
    if (fan_tray_id == 1 || fan_tray_id == 3) {
        if (mode == ONLP_LED_MODE_GREEN) {
            rc = onlp_i2c_modifyb(I2C_BUS_11, FAN_REG, offset, 0xFC,
        ...
        if (mode == ONLP_LED_MODE_GREEN) {
            rc = onlp_i2c_modifyb(I2C_BUS_11, FAN_REG, offset, 0xCF,
