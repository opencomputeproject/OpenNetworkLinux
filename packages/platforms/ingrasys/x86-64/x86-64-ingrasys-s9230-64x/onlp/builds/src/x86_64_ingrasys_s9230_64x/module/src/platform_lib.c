...
psu_pwgood_get(int *pw_good, int i2c_bus, int psu_mask)
{
    ...
    if(psu_mask == PSU1_MUX_MASK)
        psu_pg_prefix = SYS_HWMON_PSU1_PREFIX "psu_pg";
    else if(psu_mask == PSU2_MUX_MASK)
        psu_pg_prefix = SYS_HWMON_PSU2_PREFIX "psu_pg";
    else{
        return ONLP_STATUS_E_INTERNAL;
    }
    
    rv = onlp_file_read_int(&psu_pwgood, psu_pg_prefix);
    if (rv < 0) {
        return ONLP_STATUS_E_INTERNAL;
    }
...
{     
    ...
    if (port >= 1 && port <= 12) {
        i2c_dev_bus = 1;
        pd = port;
    } else if (port >= 13 && port <= 25) {
        i2c_dev_bus = 2;
        pd = port - 12;
    } else if (port >= 26 && port <= 38) {
        i2c_dev_bus = 3;
        pd = port - 25;
    } else if (port >= 39 && port <= 51) {
        i2c_dev_bus = 4;
        pd = port - 38;
    } else if (port >= 52 && port <= 64) {
        i2c_dev_bus = 5;
        pd = port - 51;
    }

    memset(data, 0, sizeof(data));
    rv = onlp_file_read(data, sizeof(data), &data_len, SYS_CPLD_QSFP_PRES_PREFIX, i2c_dev_bus, pd);

    if (rv != ONLP_STATUS_OK) {
        return ONLP_STATUS_E_INTERNAL; 
    }
    
    val = strtoul((char*)data, &endptr, 16);
    *pres_val = (val & (1<<1)) ? 0:1;
...
system_led_set(onlp_led_mode_t mode)
{
    ...
    if(mode == ONLP_LED_MODE_GREEN) {        
        rc = onlp_i2c_modifyb(I2C_BUS_10, LED_REG, LED_OFFSET, LED_SYS_AND_MASK,
...
fan_led_set(onlp_led_mode_t mode)
{
    int rc;
    if(mode == ONLP_LED_MODE_GREEN) {
        rc = onlp_i2c_modifyb(I2C_BUS_10, LED_REG, LED_OFFSET, LED_FAN_AND_MASK,
...
psu1_led_set(onlp_led_mode_t mode)
{
    int rc;
    if(mode == ONLP_LED_MODE_GREEN) {    
        rc = onlp_i2c_modifyb(I2C_BUS_10, LED_REG, LED_OFFSET, 
...
memset(cmd, 0, sizeof(cmd));
    ...
    if (fan_tray_id == 1 || fan_tray_id == 3) {
        if (mode == ONLP_LED_MODE_GREEN) {
            rc = onlp_i2c_modifyb(I2C_BUS_0, FAN_REG, offset, 0xFC,
        ...
        if (mode == ONLP_LED_MODE_GREEN) {
            rc = onlp_i2c_modifyb(I2C_BUS_0, FAN_REG, offset, 0xCF,
