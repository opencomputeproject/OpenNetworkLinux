...
onlp_sysi_platform_info_get(onlp_platform_info_t* pi)
{
    ...
    FILE *fp;
    char onl_ver_num[ONL_VER_NUM_MAX_STR_LEN]={0};
    ...
    /* Other versions - Image release version */
    fp = popen("cat /boot/version | grep 'Release Version' | awk '{print $NF}'", "r");
    fgets(onl_ver_num, sizeof (onl_ver_num),fp);
    pclose(fp);

    pi->other_versions = aim_fstrdup("%s", onl_ver_num);
