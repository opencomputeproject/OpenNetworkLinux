...
struct cpld_info_t CpldInfo[CPLD_ID_NUM] = 
{
    {.ch = CPLD_I2C_CH, 
    .addr = CPLD_I2C_ADDR},
...
struct cpld_client_t CpldClient[CPLD_ID_NUM];
...
static int sff_io_client_create_all(void)
{
    ...
    for(idx = 0; io_i2c_tbl[idx] != END_OF_TABLE; idx++) {
        if(sff_io_client_create(io_i2c_tbl[idx]) < 0)
        {
            return -1;
        }
    }    
...
static int sff_io_client_destroy_all(void)
{
    ...
    for(idx = 0; io_i2c_tbl[idx] != END_OF_TABLE; idx++)
    {
        if(sff_io_client_destroy(io_i2c_tbl[idx]) < 0)
        {
            return -1;
        }
    }
...
int ioexp_obj_set(struct ioexp_obj_t *self, int port, int val)
{
    ...
    int bit = 0; 
    int ioexp_port = 0;
    if (IS_ERR_OR_NULL(self->func->map)) {
...
static struct cpld_client_t *_cpld_client_init(int ch, u8 addr)
{
    struct i2c_client *client = NULL;
    ...
    cpld_client =  kzalloc(sizeof(struct cpld_client_t), GFP_KERNEL);

    if (IS_ERR_OR_NULL(cpld_client)) {
        goto exit_kfree_i2c_client;
    }

    cpld_client->client = client;

    mutex_init(&cpld_client->lock);
...
static int cpld_client_init(struct cpld_io_t *self)
{
    ...
    self->cpld_client = _cpld_client_init(self->ch, self->addr);
    
    if (IS_ERR_OR_NULL(self->cpld_client)) {
        return -1;
    }
    
    return 0;
}    
static int _cpld_io_deinit(struct cpld_io_t *self)
{
    struct i2c_client *client = NULL;
    struct cpld_client_t *cpld_client = NULL;
    if (!self) {
    ...
    cpld_client = self->cpld_client;
    if (cpld_client) {
    
        client = cpld_client->client;
        if (client) {
...
int cpld_ioexp_int_enable(struct cpld_io_t *self)
{
    ...
    client = self->cpld_client->client;  
    lock = &(self->cpld_client->lock);
    
    mutex_lock(lock);
    ret = inv_i2c_smbus_write_byte_data(client, self->ioexp_int_enable_offset, 0x01);
    mutex_unlock(lock);
    if (ret < 0)
    {
        return ret;
...
int cpld_ioexp_int_status_get_type1(struct cpld_io_t *self, int *reg)
{
    ...
    mutex_lock(lock);
    ret = inv_i2c_smbus_read_word_data(client, self->ioexp_int_status_offset);
    mutex_unlock(lock);
    if (ret < 0)
    {
...
int cpld_ioexp_int_status_get_type2(struct cpld_io_t *self, int *reg)
{
    ...
    mutex_lock(lock);
    ret = inv_i2c_smbus_read_byte_data(client, self->ioexp_int_status_offset);
    mutex_unlock(lock);
    if (ret < 0)
    {
        SFF_IO_ERR("2 read fail\n");
...
/*   
int cpld_ioexp_int_status_clear_type2(struct cpld_io_t *self)
{
    int ret = 0;
    int qsfp = 0;
    struct i2c_client *client = NULL; 
    ...
    mutex_lock(lock);
    ret = inv_i2c_smbus_write_byte_data(client, self->ioexp_int_clear_offset, 0xf);
    mutex_unlock(lock);
    if (ret < 0)
    {
        SFF_IO_ERR("2 write fail\n");
        return ret;
    }
...
int qsfp_ioexp_int_handler(struct sff_io_t *self)
{
    ...
    if (IS_ERR_OR_NULL(self->input_table)) {
    ...
    if (val == 0) {
        ...
        for (idx = 0; idx < size; idx++) {
            ...
            if (input_change & (1 << idx)) {
                ...
                if (IS_ERR_OR_NULL(io_prsL) || 
                    IS_ERR_OR_NULL(io_intL)) {
...
int  sfp_qsfp_ioexp_polling(struct sff_io_t *self)
{
    ...
    for (idx = 0; idx < size -1; idx++) {
        ...
        if (IS_ERR_OR_NULL(ioexp[IO_PRESENT]) || 
            IS_ERR_OR_NULL(ioexp[IO_RX_LOS]) ||
            IS_ERR_OR_NULL(ioexp[IO_TX_FAULT])) {
...
static int _cpld_io_init(struct cpld_io_t *self)
{
    ...
    return self->ioexp_int_enable(self);

}   
static void cpld_io_deinit(void)
{
    CpldIo.deinit(&CpldIo);   
}    

static int cpld_io_init(void)
{
...
int ioexp_funcs_init(void)
{
    ...
    for (i = IO_PRESENT; i < IO_IDX_NUM; i++) {
       ...
       if (IS_ERR_OR_NULL(ioexp_obj[i].st)) {
            goto exit_kfree_st;
       } 
    }
...
static int table_load(void)
{
    ...
    if (io_info->cpld_io_info) { /*ioexp interrupt mode supported*/
        ...
        CpldIo.ch = io_info->cpld_io_info->ch;
        CpldIo.addr = io_info->cpld_io_info->addr;

        CpldIo.init = _cpld_io_init;  
        CpldIo.deinit = _cpld_io_deinit; 
...
static int table_load(void)
{
    ...
    if (ioexp_func_map_load(ioexp_obj, maple_ioexp_func_map) < 0) {

        return -1;
    }

    SffIo.input_table = &maple_input_change_table;
    CpldIo.cpld_client = &CpldClient[CPLD_ID1];
    
    /*by feature*/
    #if defined (IOEXP_INT_SUPPORTED)
    SffIo.input_handler =  sfp_qsfp_ioexp_int_handler;
    /*initialize cpld io structure*/
    CpldIo.ioexp_int_status_offset = 0x35;
    CpldIo.ioexp_int_enable_offset = 0x37;
    CpldIo.ioexp_int_status_get = cpld_ioexp_int_status_get_type1; 
    //CpldIo.ioexp_int_status_clear = cpld_ioexp_no_support; 
    CpldIo.ioexp_int_enable = cpld_ioexp_int_enable; 
    CpldIo.int_gpio = CPLD_IOEXP_INTR_N; 
    Cpld.init = _cpld_io_init;  
    #else
    SffIo.input_handler =  sfp_qsfp_ioexp_polling;
    CpldIo.init = cpld_ioexp_no_support; 
    CpldIo.ioexp_int_enable = cpld_ioexp_no_support; 
...
#elif (PLATFORM == BANYAN)

    pca95xx_type_tbl = &banyan_pca95xx_type_tbl;
    io_i2c_tbl = banyan_io_i2c_tbl;
    
    if (ioexp_func_map_load(ioexp_obj, banyan_ioexp_func_map) < 0) {

        return -1;
    }

    SffIo.input_table = &banyan_input_change_table;
    SffIo.input_handler = qsfp_ioexp_int_handler;
    /*initialize cpld io structure*/
    
    CpldIo.cpld_client = &CpldClient[CPLD_ID1];
    CpldIo.ioexp_int_status_offset = 0x1d; 
    //CpldIo.ioexp_int_clear_offset = 0x19;
    CpldIo.ioexp_int_enable_offset = 0;
    CpldIo.ioexp_int_status_get = cpld_ioexp_int_status_get_type2;
    //CpldIo.ioexp_int_status_clear = cpld_ioexp_int_status_clear_type2; 
    CpldIo.ioexp_int_enable = cpld_ioexp_no_support; 
    CpldIo.int_gpio = CPLD_IOEXP_INTR_N; 
...
int sff_io_init(void)
{
    ...
    if (ioexp_output_init() < 0) {
        SFF_IO_ERR("ioexp_out_init fail\n");
        return -1;
    }
#if 1
    /*interrupt related funtions as below*/
    if (cpld_io_init() < 0) {
...
#if 0
static int __init sff_io_init(void)
{
    return retval;
}

static void __exit sff_io_deinit(void)
{
     sff_io_obj_destroy_all(); 
     sff_io_client_destroy_all();
}

module_init(sff_io_init);
module_exit(sff_io_deinit);
#endif
MODULE_LICENSE("GPL");
