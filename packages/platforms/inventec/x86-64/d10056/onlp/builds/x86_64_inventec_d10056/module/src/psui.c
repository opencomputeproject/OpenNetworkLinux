...
onlp_psui_info_get(onlp_oid_id_t id, onlp_psu_info_t* info)
{
    ...
    *info = __onlp_psu_info[id]; /* Set the onlp_oid_hdr_t */
    ...
    ONLP_TRY (ONLP_OID_STATUS_FLAG_NOT_SET(hdr,PRESENT));
    ...
    int i=0;
    for (i=0; i<8; i++) {
        char path2[ONLP_CONFIG_INFO_STR_MAX];
        snprintf(path2,ONLP_CONFIG_INFO_STR_MAX,"%spsoc_psu%d_%s",info_path,id,path_list[i]);
        if(i<2) {
            ONLP_TRY(onlp_file_read_str_dst( list1[i], ONLP_CONFIG_INFO_STR_MAX, path2 ));
        } else {
            ONLP_TRY(onlp_file_read_int( list2[i], path2 ));
        }
    }
    return ret;
}
...
int inv_psui_status_get(onlp_oid_id_t id, uint32_t* rv)
{
    ...
    ONLP_TRY( ONLP_OID_ID_VALIDATE_RANGE(id,ONLP_PSU_1,ONLP_PSU_2) );
