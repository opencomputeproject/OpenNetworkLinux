...
fans_control_log(char *buf, int fan_id)
{
    unsigned int statebit1, statebit2, bitshift;
    ...
    if ((statebit1 = inventec_singlechar_to_int(buf[2])) == -1) {
	SYSFS_LOG("[p_thread] Error value read from fan %d\n", fan_id);
	return 1;
    }
    if (buf[2] == 'f' || buf[2] == 'F') statebit1 = 0;
    ...
    bitshift = fan_id;
    //SYSFS_LOG("[p_thread] 1: statebit1 = 0x%x statebit2 = 0x%x bitshift = 0x%x\n",statebit1,statebit2,bitshift);
    if ((statebit1 & 1<<bitshift) && (statebit2 & 1<<bitshift)) {
	if(!FAN_STATE_CHECK(fan_id, FAN_STATE_BIT_UNINSTALLED)) {
	    FAN_STATE_SET(fan_id, FAN_STATE_BIT_UNINSTALLED);
	    SYSFS_LOG("[p_thread] %s: %s\n",fans_tbl[fan_id].fan_name,FAN_LOG_UNINSTALLED);
	}
