...
static int _inv_get_fan_fru(char* ret_str,int attr_type, int fan_id)
{
    ...
    snprintf(file_path,ONLP_CONFIG_INFO_STR_MAX,"%s%d-00%d/eeprom",INV_PSU_BASE,FAN_I2C_CHANNEL,(FAN_I2C_ADDR_BASE+offset-1) );
    ...
    if(rv == ONLP_STATUS_OK) {
        if(info->status & ONLP_FAN_STATUS_PRESENT) {
            ...
            case ONLP_FAN_10:
                ...
                if( _inv_get_fan_fru(info->serial,TLV_ATTR_TYPE_SERIAL,fan_id) != ONLP_STATUS_OK) {
                    snprintf(info->serial,ONLP_CONFIG_INFO_STR_MAX,"N/A");
                }
                if( _inv_get_fan_fru(info->model,TLV_ATTR_TYPE_MODEL,fan_id) != ONLP_STATUS_OK) {
...
static int _fani_status_present_check(uint32_t* status, int fan_id)
{
    ...
    if(fan_id >= ONLP_FAN_1 && fan_id <= ONLP_FAN_10) {
