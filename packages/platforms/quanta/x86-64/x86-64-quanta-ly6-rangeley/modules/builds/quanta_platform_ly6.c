...
#define DRIVER_NAME			"quanta-platform-ly6"
...
static struct platform_driver quanta_platfom_ly6_platform_driver = {
	.driver = {
		.name = DRIVER_NAME,
		.owner = THIS_MODULE,
	},
...
static int __init quanta_platfom_ly6_init(void)
{
	...
	ret = platform_driver_register(&quanta_platfom_ly6_platform_driver);
	if (ret < 0)
		return ret;
...
static void __exit quanta_platfom_ly6_cleanup(void)
{
	int i;
    /**
	...
	platform_driver_unregister(&quanta_platfom_ly6_platform_driver);
}

module_init(quanta_platfom_ly6_init);
module_exit(quanta_platfom_ly6_cleanup);

MODULE_AUTHOR("Jonathan Tsai (jonathan.tsai@quantatw.com)");
MODULE_VERSION("1.0");
MODULE_DESCRIPTION("Quanta Platform LY6");
MODULE_LICENSE("GPL");
