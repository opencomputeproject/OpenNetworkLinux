...
/* LED_PSU1 (not map to driver, after power CPLD upgrade to 0x2 version) */
#define PSU1_LED_MODE_AMBER_BLINKING        0x0     /* PSU Present */
#define PSU1_LED_MODE_GREEN                 0x1
#define PSU1_LED_MODE_OFF                   0x2     /* PSU not Present */

/* LED_PSU2 (not map to driver, after power CPLD upgrade to 0x2 version) */
#define PSU2_LED_MODE_AMBER_BLINKING        0x0     /* PSU Present */
#define PSU2_LED_MODE_GREEN                 0x1
#define PSU2_LED_MODE_OFF                   0x2     /* PSU not Present */
...
static char *leds[] =  /* must map with onlp_led_id (platform_lib.h) */
{
    ...
    "led_psu1",   /* LED_PSU1   : 0: amber blinking (PSU Present) or LED OFF (PSU not Present) , 1: green solid */
    "led_psu2",   /* LED_PSU2   : 0: amber blinking (PSU Present) or LED OFF (PSU not Present) , 1: green solid */
    "",           /* LED_SYSTEM */
    "",           /* LED_FAN    */
};
...
{
    uint8_t ledid;
    uint16_t gpioid_color_green;
	uint16_t gpioid_color_amber;
};
...
struct led_gpio_id gpio_leds[] = {
    ...
    {  },  /* Not used */
    {  },  /* Not used */
    { LED_SYSTEM, 496, 497 },  /* 496:GREEN. 497:AMBER */
    { LED_FAN,    498, 499 },  /* 498:GREEN. 499:AMBER */
};
    ...
    *info = linfo[ONLP_OID_ID_GET(id)];
    ...
    switch (lid)
    {
        ...
        case LED_PSU1:
            ...
            /*  value = 1 => Solid Green
                value = 0 => Amber blinking (PSU Present) or LED OFF (PSU not Present)
             */

            if (value == PSU1_LED_MODE_GREEN)
            {
                info->mode = driver_to_onlp_led_mode(lid, value);
            }
            else
            {
                ...
                if (onlp_psui_info_get(ONLP_PSU_ID_CREATE(PSU1_ID), &psu_info) != ONLP_STATUS_OK) 
                {
                ...
                {
                    /* PSU not Present */
                    value = PSU2_LED_MODE_OFF;
                }
                else
                {
                    /* PSU Present */
                    value = PSU2_LED_MODE_AMBER_BLINKING;
                }
        ...
        case LED_PSU2:
            ...
            else
            {
                 ...
                 */
                onlp_psu_info_t psu_info;
                if (onlp_psui_info_get(ONLP_PSU_ID_CREATE(PSU2_ID), &psu_info) != ONLP_STATUS_OK) 
                {
                    AIM_LOG_ERROR("Unable to get psu(%d) status\r\n", PSU2_ID);
                    return ONLP_STATUS_E_INTERNAL;
                }
        ...
        case LED_SYSTEM:
        case LED_FAN:
            if (onlp_file_read_int(&green_value, LED_GPIO_FORMAT, gpio_leds[lid].gpioid_color_green) < 0) 
            {
                AIM_LOG_ERROR("Unable to read status from file "LED_GPIO_FORMAT, gpio_leds[lid].gpioid_color_green);
                return ONLP_STATUS_E_INTERNAL;
            }
            if (onlp_file_read_int(&amber_value, LED_GPIO_FORMAT, gpio_leds[lid].gpioid_color_amber) < 0) 
            {
                AIM_LOG_ERROR("Unable to read status from file "LED_GPIO_FORMAT, gpio_leds[lid].gpioid_color_amber);
                return ONLP_STATUS_E_INTERNAL;
            }
...
onlp_ledi_mode_set(onlp_oid_t id, onlp_led_mode_t mode)
{
    ...
    switch (lid)
    {
        ...
        case LED_FAN:
			...
			if (onlp_file_write_int(gpio_led_map[gpio_led_index].green_value, LED_GPIO_FORMAT, gpio_leds[lid].gpioid_color_green) < 0)
    		{
        		AIM_LOG_ERROR("Unable to read status from file "LED_GPIO_FORMAT, gpio_leds[lid].gpioid_color_green);
				return ONLP_STATUS_E_INTERNAL;
    		}

			if (onlp_file_write_int(gpio_led_map[gpio_led_index].amber_value, LED_GPIO_FORMAT, gpio_leds[lid].gpioid_color_amber) < 0)
    		{
        		AIM_LOG_ERROR("Unable to read status from file "LED_GPIO_FORMAT, gpio_leds[lid].gpioid_color_amber);
				return ONLP_STATUS_E_INTERNAL;
    		}
