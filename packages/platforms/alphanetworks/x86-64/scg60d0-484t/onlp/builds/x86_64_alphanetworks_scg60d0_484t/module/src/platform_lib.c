...
{
    char *node = NULL;
    char  serial_data[PSU_SERIAL_NUMBER_LEN + 1] = {0};
    ...
    if (deviceNodeReadString(node, serial_data, sizeof(serial_data), 0) != 0) 
    {
        return ONLP_STATUS_E_INTERNAL;
    }

    aim_strlcpy(serial, serial_data, PSU_SERIAL_NUMBER_LEN + 1);
...
int psu_dps550ab_pmbus_info_get(int id, char *node, int *value)
{
    char *prefix = NULL;
    *value = 0;
...
int psu_dps550ab_pmbus_info_set(int id, char *node, int value)
{
    char *prefix = NULL;
