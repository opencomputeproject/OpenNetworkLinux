...
static int
psu_dps550ab_info_get(onlp_psu_info_t *info)
{
    ...
    if (psu_dps550ab_pmbus_info_get(index, "psu_v_out", &val) == 0) {
        info->mvout = val;
        info->caps |= ONLP_PSU_CAPS_VOUT;
    }

    if (psu_dps550ab_pmbus_info_get(index, "psu_v_in", &val) == 0) {
        info->mvin = val;
        info->caps |= ONLP_PSU_CAPS_VIN;
    }

    if (psu_dps550ab_pmbus_info_get(index, "psu_i_out", &val) == 0) {
        info->miout = val;
        info->caps |= ONLP_PSU_CAPS_IOUT;
    }
    
    if (psu_dps550ab_pmbus_info_get(index, "psu_i_in", &val) == 0) {
        info->miin = val;
        info->caps |= ONLP_PSU_CAPS_IIN;
    }

    if (psu_dps550ab_pmbus_info_get(index, "psu_p_out", &val) == 0) {
        info->mpout = val;
        info->caps |= ONLP_PSU_CAPS_POUT;
    }   

    if (psu_dps550ab_pmbus_info_get(index, "psu_p_in", &val) == 0) {
        info->mpin = val;
...
{
    { }, /* Not used */
    {    /* PSU-1 is on i2c channel 5 for scg60d0*/
        { ONLP_PSU_ID_CREATE(PSU1_ID), "PSU-1", 0 },
    },
    {    /* PSU-2 is on i2c channel 6 for scg60d0*/
...
onlp_psui_info_get(onlp_oid_t id, onlp_psu_info_t* info)
{
    ...
    *info = pinfo[index]; /* Set the onlp_oid_hdr_t */
    ...
    case PSU_TYPE_AC_B2F:
        ret = psu_dps550ab_info_get(info);
        break;
