...
struct snj61d0_320f_onie_eeprom_data
{
...
enum snj61d0_320f_onie_eeprom_sysfs_attributes {
    ONIE_RW,
};
...
static ssize_t onie_read(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    struct snj61d0_320f_onie_eeprom_data *data = i2c_get_clientdata(client);
...
{
    struct snj61d0_320f_onie_eeprom_data *data;
    int status;
    ...
    data = kzalloc(sizeof(struct snj61d0_320f_onie_eeprom_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
