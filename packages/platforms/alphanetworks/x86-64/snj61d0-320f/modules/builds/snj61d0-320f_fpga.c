/*
 * A hwmon driver for the alphanetworks_snj61d0_320f_fpga
 *
...
#define SYS_PSU1_LED_BITS       0x38
#define SYS_PSU2_LED_BITS       0x07
...
/* Addresses scanned for alphanetworks_snj61d0_320f_fpga */
static const unsigned short normal_i2c[] = { 0x5E, I2C_CLIENT_END };

struct alphanetworks_snj61d0_320f_pwr_fpga_data {
...
static SENSOR_DEVICE_ATTR(sys_psu1_led,(0660), sys_led_read, sys_led_write, SYS_PSU1_LED);
static SENSOR_DEVICE_ATTR(sys_psu2_led,(0660), sys_led_read, sys_led_write, SYS_PSU2_LED);
...
static struct attribute *alphanetworks_snj61d0_320f_fpga_attributes[] = {
  ...
  &sensor_dev_attr_sys_psu1_led.dev_attr.attr,
  &sensor_dev_attr_sys_psu2_led.dev_attr.attr,
...
static const struct attribute_group alphanetworks_snj61d0_320f_fpga_group = {
  .attrs = alphanetworks_snj61d0_320f_fpga_attributes,
};
...
static ssize_t sys_led_read(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    switch(sda->index) {
        case SYS_LOCATOR:
          ...
          break;
        case SYS_PSU1_LED:
          res = (val & SYS_PSU1_LED_BITS) >> 3;
          break;
        case SYS_PSU2_LED:
          res = (val & SYS_PSU2_LED_BITS) >> 0;
          break;
        case PORT_LED_DISABLE:
...
static ssize_t sys_led_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    switch(sda->index) {
        case SYS_LOCATOR:
          ...
          break;
        case SYS_PSU1_LED:
          read &= ~SYS_PSU1_LED_BITS;
          read |= write << 3;
          break;
        case SYS_PSU2_LED:
          read &= ~SYS_PSU2_LED_BITS;
          read |= write << 0;
          break;
        case PORT_LED_DISABLE:
...
{
    int status;
    struct alphanetworks_snj61d0_320f_pwr_fpga_data* data;
    ...
    data = kzalloc(sizeof(struct alphanetworks_snj61d0_320f_pwr_fpga_data), GFP_KERNEL);
    if (!data) {
      status = -ENOMEM;
      goto exit;
    }

    status = sysfs_create_group(&client->dev.kobj, &alphanetworks_snj61d0_320f_fpga_group);
    if (status) {
      goto exit;
    }
...
{
    struct alphanetworks_snj61d0_320f_pwr_fpga_data *data = i2c_get_clientdata(client);
    sysfs_remove_group(&client->dev.kobj, &alphanetworks_snj61d0_320f_fpga_group);
