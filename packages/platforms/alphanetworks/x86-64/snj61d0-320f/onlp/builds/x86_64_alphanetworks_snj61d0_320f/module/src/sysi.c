...
#include "x86_64_alphanetworks_snj61d0_320f_int.h"
...
#define PLATFORM_STRING "x86-64-alphanetworks-snj61d0-320f-r2"
...
{
    DIAG_PRINT("%s", __FUNCTION__);
    int i = 0, j = 0, fan_fault = 0, psu_fault = 0;
    ...
    /* Get each psu status
     */
    for (i = PSU1_ID, j = LED_PSU1; i <= CHASSIS_PSU_COUNT; i++, j++)
    {
        ...
        {
            /* Get PSU status fail */
            onlp_ledi_mode_set(ONLP_LED_ID_CREATE(j), ONLP_LED_MODE_ORANGE_BLINKING);
            AIM_LOG_ERROR("Unable to get psu(%d) status\r\n", i);                     
            return ONLP_STATUS_E_INTERNAL;
        }
      
        if ((psu_info.status & ONLP_PSU_STATUS_PRESENT) == 0)
        {
            /* Not present */
            onlp_ledi_mode_set(ONLP_LED_ID_CREATE(j), ONLP_LED_MODE_OFF);
        ...
        {
            /* Normal Working */
            onlp_ledi_mode_set(ONLP_LED_ID_CREATE(j), ONLP_LED_MODE_GREEN);
        }
    }
...
int onlp_sysi_debug_diag_sfp(int index)
{
    ...
    if (index >= SFP_START_INDEX && index <= (SFP_START_INDEX+NUM_OF_SFP_PORT))
    {
        ...
        else if (IS_QSFP_PORT(index))
        {
            addr = SYSTEM_FPGA_I2C_ADDR;
