...
/* Power LED and PSU LED are controlled by CPLD */
#define CPLD_PSU_LED_ADDRESS_OFFSET      0x0A //PSU0 [1:0], PSU1 [3:2]
#define CPLD_POWER_LED_ADDRESS_OFFSET    0x0B //power [1:0]

#define PCA9539_NUM1_I2C_BUS_ID          1
#define PCA9539_NUM1_I2C_ADDR            0x75 /* PCA9539#1 Physical Address in the I2C */

#define PCA9539_NUM1_IO0_INPUT_OFFSET    0x00 /* used to read data */
#define PCA9539_NUM1_IO0_OUTPUT_OFFSET   0x02 /* used to write data*/
...
#define PCA9539_NUM1_SYSTEM_LED_Y_BIT    0
#define PCA9539_NUM1_SYSTEM_LED_G_BIT    1
#define PCA9539_NUM1_FAN_LED_Y_BIT       2
#define PCA9539_NUM1_FAN_LED_G_BIT       3
#define PCA9539_NUM1_LOC_LED_B_BIT       4
...
/* 
  ...
  * 4. Assign VSB_PG_UCDGPI2 <= V3P3_SB_PG and V2P5_SB_PG and V1P2_SB_PG and V1P15_SB_PG and V1P8_SB_PG                                                                                            -- > Bit[0] = 1, PWR_LED_Y is off
  *
  */
...
#define SYSTEM_LED_OFF                     0x03
#define SYSTEM_LED_AMBER_SOLID             0x02
#define SYSTEM_LED_GREEN_SOLID             0x01
#define SYSTEM_LED_GREEN_BLINKING          0x00 

#define FAN_LED_OFF                     0x03
#define FAN_LED_AMBER_SOLID             0x02
#define FAN_LED_GREEN_SOLID             0x01

#define LOC_LED_OFF                     0x01
#define LOC_LED_BLUE_BLINKING           0x00
...
static struct led_id_mode led_id_mode_data[] = {
    ...
    { LED_PSU0, ONLP_LED_MODE_OFF, PSU_LED_OFF },
    { LED_PSU0, ONLP_LED_MODE_GREEN, PSU_LED_GREEN_SOLID },
    { LED_PSU0, ONLP_LED_MODE_ORANGE_BLINKING, PSU_LED_AMBER_BLINKING_BIT0_L},
    { LED_PSU0, ONLP_LED_MODE_ORANGE_BLINKING, PSU_LED_AMBER_BLINKING_BIT0_H},    
    { LED_PSU0, ONLP_LED_MODE_ON, PSU_LED_GREEN_SOLID },
...
}_PCA9539_NUM1_SYS_FAN_LED_REG_T;
...
static onlp_led_info_t linfo[] =
{
    ...
    {
        { ONLP_LED_ID_CREATE(LED_PSU0), "Chassis LED 2 (PSU0 LED)", 0 },
        ONLP_LED_STATUS_PRESENT,
        ONLP_LED_CAPS_ON_OFF | ONLP_LED_CAPS_GREEN| ONLP_LED_CAPS_ORANGE_BLINKING,
    },
    {
...
static void *set_system_led_green_blinking(void *arg)
{
    ...
    _PCA9539_NUM1_SYS_FAN_LED_REG_T sys_fan_led_reg;

	system_led_off = SYSTEM_LED_OFF;
	system_led_green_solid = SYSTEM_LED_GREEN_SOLID;

    do {
      data = onlp_i2c_readb(PCA9539_NUM1_I2C_BUS_ID, PCA9539_NUM1_I2C_ADDR, PCA9539_NUM1_IO0_INPUT_OFFSET, ONLP_I2C_F_FORCE);
      if (data < 0)
      {
      ...
      ret = onlp_i2c_writeb(PCA9539_NUM1_I2C_BUS_ID, PCA9539_NUM1_I2C_ADDR, PCA9539_NUM1_IO0_OUTPUT_OFFSET, sys_fan_led_reg.val, ONLP_I2C_F_FORCE);
      if (ret < 0)
      {
...
static void *set_locator_led_blue_blinking(void *arg)
{
    int ret = 0;
    int data = 0;
    _PCA9539_NUM1_SYS_FAN_LED_REG_T locator_led_reg;
    ...
    do {
      ...
      ret = onlp_i2c_writeb(PCA9539_NUM1_I2C_BUS_ID, PCA9539_NUM1_I2C_ADDR, PCA9539_NUM1_IO0_OUTPUT_OFFSET, locator_led_reg.val, ONLP_I2C_F_FORCE);
      if (ret < 0)
      {
          AIM_LOG_INFO("%s:%d fail[%d]\n", __FUNCTION__, __LINE__, ret);
          locator_led_thread_is_running = 0;
...
spx70d0_led_pca9539_direction_set(int IO_port)
{
    ...
    data &= ~(1 << PCA9539_NUM1_SYSTEM_LED_Y_BIT);
    data &= ~(1 << PCA9539_NUM1_SYSTEM_LED_G_BIT);
    data &= ~(1 << PCA9539_NUM1_FAN_LED_Y_BIT);
    data &= ~(1 << PCA9539_NUM1_FAN_LED_G_BIT);
    data &= ~(1 << PCA9539_NUM1_LOC_LED_B_BIT);
...
spx70d0_led_pca9539_direction_setting_check(int IO_port)
{
    ...
    /* Check PCA9539#1 I/O port0 direction has been initialized or not */
...
onlp_ledi_init(void)
{
    ...
    /* Configure PCA9539#1 I/O port0 direction if not been initialized */
...
onlp_ledi_info_get(onlp_oid_t id, onlp_led_info_t *info)
{
    ...
    *info = linfo[ONLP_OID_ID_GET(id)];
    ...
    {
        case LED_POWER:			
			ret = onlp_i2c_readb(POWERCPLD_I2C_BUS_ID, POWERCPLD_I2C_ADDR, CPLD_POWER_LED_ADDRESS_OFFSET, ONLP_I2C_F_FORCE);
            if (ret < 0)
            {
                AIM_LOG_INFO("%s:%d fail[%d]\n", __FUNCTION__, __LINE__, ret);
                return ret;
            }		
            power_led_reg.val = ret;
            break;

        case LED_PSU0:
		case LED_PSU1:
			ret = onlp_i2c_readb(POWERCPLD_I2C_BUS_ID, POWERCPLD_I2C_ADDR, CPLD_PSU_LED_ADDRESS_OFFSET, ONLP_I2C_F_FORCE);
            if (ret < 0)
            {
                AIM_LOG_INFO("%s:%d fail[%d]\n", __FUNCTION__, __LINE__, ret);
                return ret;
        ...
        case LED_FAN:
        case LED_LOC:    
            ret = onlp_i2c_readb(PCA9539_NUM1_I2C_BUS_ID, PCA9539_NUM1_I2C_ADDR, PCA9539_NUM1_IO0_INPUT_OFFSET, ONLP_I2C_F_FORCE);
            if (ret < 0)
            {
                AIM_LOG_INFO("%s:%d fail[%d]\n", __FUNCTION__, __LINE__, ret);
                return ret;
...
onlp_ledi_mode_set(onlp_oid_t id, onlp_led_mode_t mode)
{
    ...
    switch (local_id)
    {
        case LED_POWER:
			...
			ret = onlp_i2c_writeb(POWERCPLD_I2C_BUS_ID, POWERCPLD_I2C_ADDR, CPLD_POWER_LED_ADDRESS_OFFSET, power_led_reg.val, ONLP_I2C_F_FORCE);
            if (ret < 0)
            {
                AIM_LOG_INFO("%s:%d fail[%d]\n", __FUNCTION__, __LINE__, ret);
                return ret;
        ...
        case LED_PSU0:
		case LED_PSU1:
			ret = onlp_i2c_writeb(POWERCPLD_I2C_BUS_ID, POWERCPLD_I2C_ADDR, CPLD_PSU_LED_ADDRESS_OFFSET, psu_led_reg.val, ONLP_I2C_F_FORCE);
            if (ret < 0)
            {
                AIM_LOG_INFO("%s:%d fail[%d]\n", __FUNCTION__, __LINE__, ret);
                return ret;
