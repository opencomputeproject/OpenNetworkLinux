...
#define DRVNAME "as9926_24db_psu"
...
static ssize_t show_string(struct device *dev, struct device_attribute *attr,
							char *buf);
static int as9926_24db_psu_probe(struct platform_device *pdev);
static int as9926_24db_psu_remove(struct platform_device *pdev);
...
struct as9926_24db_psu_data *data = NULL;
...
enum as9926_24db_psu_sysfs_attrs {
	/* psu attributes */
	PSU_ATTR(1),
	PSU_ATTR(2),
	NUM_OF_PSU_ATTR,
...
#define DECLARE_PSU_SENSOR_DEVICE_ATTR(index) \
	...
	static SENSOR_DEVICE_ATTR(psu##index##_vin, S_IRUGO, show_psu, \
				  NULL, PSU##index##_VIN); \
	static SENSOR_DEVICE_ATTR(psu##index##_vout, S_IRUGO, show_psu, \
				  NULL, PSU##index##_VOUT); \
	static SENSOR_DEVICE_ATTR(psu##index##_iin, S_IRUGO, show_psu, \
				  NULL, PSU##index##_IIN); \
	static SENSOR_DEVICE_ATTR(psu##index##_iout, S_IRUGO, show_psu, \
				  NULL, PSU##index##_IOUT); \
	static SENSOR_DEVICE_ATTR(psu##index##_pin, S_IRUGO, show_psu, \
				  NULL, PSU##index##_PIN); \
	static SENSOR_DEVICE_ATTR(psu##index##_pout, S_IRUGO, show_psu, \
				  NULL, PSU##index##_POUT); \
...
static struct attribute *as9926_24db_psu_attributes[] = {
	/* psu attributes */
	DECLARE_PSU_ATTR(1),
	DECLARE_PSU_ATTR(2),
	NULL
...
static const struct attribute_group as9926_24db_psu_group = {
	.attrs = as9926_24db_psu_attributes,
};
...
static struct as9926_24db_psu_data *
as9926_24db_psu_update_device(struct device_attribute *da)
{
	...
	data->ipmi_tx_data[0] = pid + 1; /* PSU ID base id for ipmi 
					    start from 1 */
			...
			char *buf)
{
	...
	data = as9926_24db_psu_update_device(da);
	if (!data->valid[pid]) {
		error = -EIO;
		goto exit;
	}
...
static int as9926_24db_psu_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as9926_24db_psu_group);
	if (status)
		goto exit;
...
static int __init as9926_24db_psu_init(void)
{
	...
	data = kzalloc(sizeof(struct as9926_24db_psu_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&as9926_24db_psu_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&as9926_24db_psu_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit as9926_24db_psu_exit(void)
{
...
MODULE_DESCRIPTION("as9926_24db_psu driver");
MODULE_LICENSE("GPL");

module_init(as9926_24db_psu_init);
module_exit(as9926_24db_psu_exit);
