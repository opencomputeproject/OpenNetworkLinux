...
static int _onlp_fani_info_get_fan(int fid, onlp_fan_info_t* info)
{
	 ...
	 */
	if (onlp_file_read_int(&value, "%s""fan%d_present", FAN_BOARD_PATH,
	    fid) < 0) {
	...
	/* get fan direction
	 */
	len = onlp_file_read_str(&fandir, "%s""fan%d_dir", FAN_BOARD_PATH, 
				 fid);
	...
	/* get front fan speed
	 */
	if (onlp_file_read_int(&value, "%s""fan%d_input", FAN_BOARD_PATH, 
			       fid) < 0) {
		AIM_LOG_ERROR("Unable to read status from (%s)\r\n", 
			      FAN_BOARD_PATH);
		return ONLP_STATUS_E_INTERNAL;
...
static int _onlp_fani_info_get_fan_on_psu(int pid, onlp_fan_info_t* info)
{
	...
	/* get fan speed
         */
	ret = onlp_file_read_int(&value, "%s""psu%d_fan1_input",
				 PSU_SYSFS_PATH, pid);
...
int onlp_fani_percentage_set(onlp_oid_t id, int p)
{
	...
	if (onlp_file_write_int(p, "%s""fan%d_pwm", FAN_BOARD_PATH, fid) < 0) {
		AIM_LOG_ERROR("Unable to write data to file %s""fan%d_pwm", 
			      FAN_BOARD_PATH, fid);
		return ONLP_STATUS_E_INTERNAL;
	}
