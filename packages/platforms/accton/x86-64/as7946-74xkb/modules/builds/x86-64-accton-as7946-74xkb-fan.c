...
static ssize_t show_dir(struct device *dev, struct device_attribute *da,
			char *buf);
static int as7946_74xkb_fan_probe(struct platform_device *pdev);
static int as7946_74xkb_fan_remove(struct platform_device *pdev);
...
struct as7946_74xkb_fan_data *data = NULL;
...
enum as7946_74xkb_fan_sysfs_attrs {
	FAN_ATTR(1),
	FAN_ATTR(2),
	FAN_ATTR(3),
	FAN_ATTR(4),
...
static struct attribute *as7946_74xkb_fan_attributes[] = {
	/* fan attributes */
	DECLARE_FAN_ATTR(1),
	DECLARE_FAN_ATTR(2),
	DECLARE_FAN_ATTR(3),
							...
							char *buf)
{
	...
	data = as7946_74xkb_fan_update_device();
	if (!data->valid) {
		error = -EIO;
		goto exit;
	}
...
static struct as7946_74xkb_fan_data *as7946_74xkb_fan_update_cpld_ver(void)
{
	int status = 0;
								...
								char *buf)
{
	...
	data = as7946_74xkb_fan_update_cpld_ver();
	if (!data->valid) {
		error = -EIO;
		goto exit;
	}
...
static int as7946_74xkb_fan_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as7946_74xkb_fan_group);
	if (status)
		goto exit;
...
static int as7946_74xkb_fan_remove(struct platform_device *pdev)
{
	sysfs_remove_group(&pdev->dev.kobj, &as7946_74xkb_fan_group);
...
static int __init as7946_74xkb_fan_init(void)
{
	...
	data = kzalloc(sizeof(struct as7946_74xkb_fan_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&as7946_74xkb_fan_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&as7946_74xkb_fan_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit as7946_74xkb_fan_exit(void)
{
...
MODULE_DESCRIPTION("as7946_74xkb fan driver");
MODULE_LICENSE("GPL");

module_init(as7946_74xkb_fan_init);
module_exit(as7946_74xkb_fan_exit);
