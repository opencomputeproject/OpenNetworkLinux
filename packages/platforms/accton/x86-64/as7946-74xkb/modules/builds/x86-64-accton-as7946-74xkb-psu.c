...
#define DRVNAME "as7946_74xkb_psu"
...
struct as7946_74xkb_psu_data *data = NULL;
...
enum as7946_74xkb_psu_sysfs_attrs {
	/* psu attributes */
	PSU_ATTR(1),
	PSU_ATTR(2),
	NUM_OF_PSU_ATTR,
...
static struct attribute *as7946_74xkb_psu_attributes[] = {
	/* psu attributes */
	DECLARE_PSU_ATTR(1),
	DECLARE_PSU_ATTR(2),
	NULL
...
static const struct attribute_group as7946_74xkb_psu_group = {
	.attrs = as7946_74xkb_psu_attributes,
};
...
static struct as7946_74xkb_psu_data *as7946_74xkb_psu_update_device(struct device_attribute *da)
{
							...
							char *buf)
{
	...
	data = as7946_74xkb_psu_update_device(da);
	if (!data->valid[pid]) {
		error = -EIO;
		goto exit;
	}
...
static int as7946_74xkb_psu_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as7946_74xkb_psu_group);
	if (status)
		goto exit;
...
static int __init as7946_74xkb_psu_init(void)
{
	...
	data = kzalloc(sizeof(struct as7946_74xkb_psu_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&as7946_74xkb_psu_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&as7946_74xkb_psu_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit as7946_74xkb_psu_exit(void)
{
...
MODULE_DESCRIPTION("as7946_74xkb_psu driver");
MODULE_LICENSE("GPL");

module_init(as7946_74xkb_psu_init);
module_exit(as7946_74xkb_psu_exit);
