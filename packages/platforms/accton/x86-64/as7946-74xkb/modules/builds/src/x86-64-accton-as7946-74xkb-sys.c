...
#define DRVNAME "as7946_74xkb_sys"
...
struct as7946_74xkb_sys_data *data = NULL;
...
enum as7946_74xkb_sys_sysfs_attrs {
...
static const struct attribute_group as7946_74xkb_sys_group = {
	.attrs = as7946_74xkb_sys_attributes,
};
...
static struct as7946_74xkb_sys_data *
as7946_74xkb_sys_update_cpld_ver(unsigned char cpld_addr)
{
	int status = 0;
				 ...
				 struct device_attribute *da, char *buf)
{
	...
	data = as7946_74xkb_sys_update_cpld_ver(cpld_addr);
	if (!data->valid) {
		error = -EIO;
		goto exit;
	}
...
static int as7946_74xkb_sys_probe(struct platform_device *pdev)
{
	...
	/* Register sysfs hooks */
	status = sysfs_create_group(&pdev->dev.kobj, &as7946_74xkb_sys_group);
	if (status) {
		goto exit;
	}
...
static int __init as7946_74xkb_sys_init(void)
{
	...
	data = kzalloc(sizeof(struct as7946_74xkb_sys_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&as7946_74xkb_sys_driver);
...
dev_reg_err:
	platform_driver_unregister(&as7946_74xkb_sys_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit as7946_74xkb_sys_exit(void)
{
...
MODULE_DESCRIPTION("AS7946-74XKB System driver");
MODULE_LICENSE("GPL");

module_init(as7946_74xkb_sys_init);
module_exit(as7946_74xkb_sys_exit);