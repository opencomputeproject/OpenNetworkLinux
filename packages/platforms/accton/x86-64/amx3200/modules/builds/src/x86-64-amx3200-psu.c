...
static ssize_t show_string(struct device *dev, struct device_attribute *attr,
                            char *buf);
static int amx3200_psu_probe(struct platform_device *pdev);
static int amx3200_psu_remove(struct platform_device *pdev);
...
struct amx3200_psu_data *data = NULL;

static struct platform_driver amx3200_psu_driver = {
    .probe = amx3200_psu_probe,
    .remove = amx3200_psu_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
static struct attribute *amx3200_psu1_attrs[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(1),
...
static struct attribute_group amx3200_psu1_group = {
    .attrs = amx3200_psu1_attrs,
};
/* ATTRIBUTE_GROUPS(amx3200_psu1); */
...
static struct attribute *amx3200_psu2_attrs[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(2),
...
static struct attribute_group amx3200_psu2_group = {
    .attrs = amx3200_psu2_attrs,
};
/* ATTRIBUTE_GROUPS(amx3200_psu2); */

const struct attribute_group *amx3200_psu_groups[][2] = {
    {&amx3200_psu1_group, NULL},
    {&amx3200_psu2_group, NULL}
};
...
static struct amx3200_psu_data *amx3200_psu_update_device(struct device_attribute *da)
{
                            ...
                            char *buf)
{
    ...
    data = amx3200_psu_update_device(da);
    if (!data->valid[pid]) {
        error = -EIO;
        goto exit;
    }
...
static int amx3200_psu_probe(struct platform_device *pdev)
{
    ...
    hwmon_dev = hwmon_device_register_with_info(&pdev->dev, DRVNAME, 
                    NULL, NULL, amx3200_psu_groups[pdev->id]);
    if (IS_ERR(hwmon_dev)) {
...
static int __init amx3200_psu_init(void)
{
    int ret;
    int i;

    data = kzalloc(sizeof(struct amx3200_psu_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&amx3200_psu_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&amx3200_psu_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit amx3200_psu_exit(void)
{
    int i;
...
MODULE_DESCRIPTION("amx3200_psu driver");
MODULE_LICENSE("GPL");

module_init(amx3200_psu_init);
module_exit(amx3200_psu_exit);
