...
static ssize_t show_threshold(struct device *dev, struct device_attribute *da,
            char *buf);
static int amx3200_fan_probe(struct platform_device *pdev);
static int amx3200_fan_remove(struct platform_device *pdev);
...
struct amx3200_fan_data *data = NULL;

static struct platform_driver amx3200_fan_driver = {
    .probe = amx3200_fan_probe,
    .remove = amx3200_fan_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
static struct attribute *amx3200_fan_attrs[] = {
    /* fan attributes */
    DECLARE_FAN_ATTR(1),
    DECLARE_FAN_ATTR(2),
    DECLARE_FAN_ATTR(3),
...
static struct amx3200_fan_data *amx3200_fan_update_device(void)
{
    int status = 0;
                            ...
                            char *buf)
{
    ...
    data = amx3200_fan_update_device();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
            ...
            const char *buf, size_t count)
{
     ...
     * Since NUM_OF_FAN_MODULE is 10 in AMX3200, it needs to be divided by 2.
     * The result :
     ...
     * fan5_pwm(Front) : ipmi_tx_data[0] = 5
     ...
     * fan10_pwm(Rear) : ipmi_tx_data[0] = 5
     */
...
static struct amx3200_fan_data *amx3200_fan_update_cpld_ver(void)
{
    int status = 0;
                                ...
                                char *buf)
{
    ...
    data = amx3200_fan_update_cpld_ver();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static int __init amx3200_fan_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct amx3200_fan_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&amx3200_fan_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&amx3200_fan_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit amx3200_fan_exit(void)
{
    if (data) {
...
MODULE_DESCRIPTION("amx3200_fan driver");
MODULE_LICENSE("GPL");

module_init(amx3200_fan_init);
module_exit(amx3200_fan_exit);
