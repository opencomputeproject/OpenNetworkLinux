...
static ssize_t show_led(struct device *dev, struct device_attribute *attr,
            char *buf);
static int amx3200_led_probe(struct platform_device *pdev);
static int amx3200_led_remove(struct platform_device *pdev);
...
struct amx3200_led_data *data = NULL;

static struct platform_driver amx3200_led_driver = {
    .probe = amx3200_led_probe,
    .remove = amx3200_led_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
enum ipmi_led_light_mode {
    ...
    IPMI_LED_MODE_AMBER_BLINKING = 0x11,
    IPMI_LED_MODE_ORANGE = 0x20,
    IPMI_LED_MODE_ORANGE_BLINKING = 0x21,
};
...
enum led_light_mode {
    ...
    LED_MODE_AMBER = 28,
    LED_MODE_AMBER_BLINKING = 29,
    LED_MODE_UNKNOWN = 99
};

enum amx3200_led_sysfs_attrs {
    LED_LOC,
    LED_DIAG,
    LED_FAN,
    LED_PSU
...
static SENSOR_DEVICE_ATTR(led_fan, S_IWUSR | S_IRUGO, show_led, NULL,
                            LED_FAN);
static SENSOR_DEVICE_ATTR(led_psu, S_IWUSR | S_IRUGO, show_led, NULL,
                            LED_PSU);

static struct attribute *amx3200_led_attributes[] = {
...
static const struct attribute_group amx3200_led_group = {
    .attrs = amx3200_led_attributes,
};
...
static struct amx3200_led_data *amx3200_led_update_device(void)
{
    int status = 0;
                            ...
                            char *buf)
{
    ...
    data = amx3200_led_update_device();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
    ...
    case IPMI_LED_MODE_AMBER:
        value = LED_MODE_AMBER;
        break;
    case IPMI_LED_MODE_AMBER_BLINKING:
        value = LED_MODE_AMBER_BLINKING;
        break;
            ...
            const char *buf, size_t count)
{
    ...
    case LED_MODE_AMBER_BLINKING:
        data->ipmi_resp[1] = IPMI_LED_MODE_AMBER_BLINKING;
        break;
...
static int amx3200_led_probe(struct platform_device *pdev)
{
    ...
    status = sysfs_create_group(&pdev->dev.kobj, &amx3200_led_group);
    if (status)
        goto exit;
...
static int amx3200_led_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &amx3200_led_group);

    return 0;
}

static int __init amx3200_led_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct amx3200_led_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&amx3200_led_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&amx3200_led_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit amx3200_led_exit(void)
{
...
MODULE_DESCRIPTION("amx3200_led driver");
MODULE_LICENSE("GPL");

module_init(amx3200_led_init);
module_exit(amx3200_led_exit);
