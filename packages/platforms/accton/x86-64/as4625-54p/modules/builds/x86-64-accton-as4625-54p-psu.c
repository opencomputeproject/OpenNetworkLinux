...
static struct as4625_54p_psu_data 
	*as4625_54p_psu_update_device(struct device *dev);
...
static int as4625_54p_psu_read_byte(struct i2c_client *client, u8 command,
					 u8 *data)
{
	int status = 0;
	int retry_count = 5;
...
static int as4625_54p_psu_read_bytes(struct i2c_client *client, u8 command, 
...
static struct as4625_54p_psu_data *as4625_54p_psu_update_device(
						struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct as4625_54p_psu_data *data = i2c_get_clientdata(client);
	...
	if (time_after(jiffies, data->last_updated + HZ + HZ / 2)
		...
		if (IS_POWER_GOOD(data->index, data->status)) {
			/* Read model name */
			status = as4625_54p_psu_read_bytes(client, 
			...
			if (status < 0) {
				data->serial[0] = '\0';
				dev_dbg(&client->dev, 
					"unable to read serial number from (0x%x)\n", 
					client->addr);
				goto exit;
			} else {
			...
			if (strncmp(data->model_name, "UP1K21R-1085G", 
				ARRAY_SIZE(data->model_name)-1) == 0)
				memcpy(data->fan_dir, FAN_DIR_F2B, 
					sizeof(FAN_DIR_F2B));
			else
				data->fan_dir[0] = '\0';
		}
