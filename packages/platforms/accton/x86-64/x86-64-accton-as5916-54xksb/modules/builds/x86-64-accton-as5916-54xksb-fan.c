...
#define DRVNAME "as5916_54xksb_fan"
...
static int as5916_54xksb_fan_probe(struct platform_device *pdev);
static int as5916_54xksb_fan_remove(struct platform_device *pdev);
...
struct as5916_54xksb_fan_data *data = NULL;

static struct platform_driver as5916_54xksb_fan_driver = {
    .probe      = as5916_54xksb_fan_probe,
    .remove     = as5916_54xksb_fan_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
static struct attribute *as5916_54xksb_fan_attributes[] = {
    /* fan attributes */
    DECLARE_FAN_ATTR(1),
    DECLARE_FAN_ATTR(2),
    DECLARE_FAN_ATTR(3),
...
static const struct attribute_group as5916_54xksb_fan_group = {
    .attrs = as5916_54xksb_fan_attributes,
};
...
static struct as5916_54xksb_fan_data *as5916_54xksb_fan_update_device(void)
{
    int status = 0;
...
static ssize_t show_fan(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = as5916_54xksb_fan_update_device();
    if (!data->valid) {
        return -EIO;
    }
...
static int as5916_54xksb_fan_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as5916_54xksb_fan_group);
	if (status) {
		goto exit;
	}
...
static int as5916_54xksb_fan_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as5916_54xksb_fan_group);

    return 0;
}

static int __init as5916_54xksb_fan_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as5916_54xksb_fan_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as5916_54xksb_fan_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&as5916_54xksb_fan_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as5916_54xksb_fan_exit(void)
{
...
MODULE_DESCRIPTION("AS5916 54XKSB fan driver");
MODULE_LICENSE("GPL");

module_init(as5916_54xksb_fan_init);
module_exit(as5916_54xksb_fan_exit);
