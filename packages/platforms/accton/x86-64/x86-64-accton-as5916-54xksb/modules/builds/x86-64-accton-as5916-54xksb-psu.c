...
#define DRVNAME "as5916_54xksb_psu"
...
static int as5916_54xksb_psu_probe(struct platform_device *pdev);
static int as5916_54xksb_psu_remove(struct platform_device *pdev);
...
struct as5916_54xksb_psu_data *data = NULL;

static struct platform_driver as5916_54xksb_psu_driver = {
    .probe      = as5916_54xksb_psu_probe,
    .remove     = as5916_54xksb_psu_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
static struct attribute *as5916_54xksb_psu_attributes[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(1),
    DECLARE_PSU_ATTR(2),
    NULL
...
static const struct attribute_group as5916_54xksb_psu_group = {
    .attrs = as5916_54xksb_psu_attributes,
};
...
static struct as5916_54xksb_psu_data *as5916_54xksb_psu_update_device(struct device_attribute *da)
{
...
static ssize_t show_psu(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = as5916_54xksb_psu_update_device(da);
    if (!data->valid[pid]) {
        return -EIO;
    }
...
static ssize_t show_string(struct device *dev, struct device_attribute *da, char *buf)
{
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    unsigned char pid = attr->index / NUM_OF_PER_PSU_ATTR;
    struct as5916_54xksb_psu_data *data;
    char *str = NULL;
...
static int as5916_54xksb_psu_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as5916_54xksb_psu_group);
	if (status) {
		goto exit;
	}
...
static int as5916_54xksb_psu_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as5916_54xksb_psu_group);
    return 0;
}

static int __init as5916_54xksb_psu_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as5916_54xksb_psu_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as5916_54xksb_psu_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&as5916_54xksb_psu_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as5916_54xksb_psu_exit(void)
{
...
MODULE_DESCRIPTION("AS5916 54XKSB PSU driver");
MODULE_LICENSE("GPL");

module_init(as5916_54xksb_psu_init);
module_exit(as5916_54xksb_psu_exit);
