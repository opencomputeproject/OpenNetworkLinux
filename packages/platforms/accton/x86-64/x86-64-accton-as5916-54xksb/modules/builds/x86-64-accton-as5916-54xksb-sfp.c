...
#define DRVNAME "as5916_54xksb_sfp"
...
static int as5916_54xksb_sfp_probe(struct platform_device *pdev);
static int as5916_54xksb_sfp_remove(struct platform_device *pdev);
static ssize_t show_all(struct device *dev, struct device_attribute *da, char *buf);
static struct as5916_54xksb_sfp_data *as5916_54xksb_sfp_update_present(void);
static struct as5916_54xksb_sfp_data *as5916_54xksb_sfp_update_txdisable(void);
static struct as5916_54xksb_sfp_data *as5916_54xksb_sfp_update_txfault(void);
static struct as5916_54xksb_sfp_data *as5916_54xksb_sfp_update_rxlos(void);
static struct as5916_54xksb_sfp_data *as5916_54xksb_qsfp_update_present(void);
static struct as5916_54xksb_sfp_data *as5916_54xksb_qsfp_update_txdisable(void);
static struct as5916_54xksb_sfp_data *as5916_54xksb_qsfp_update_reset(void);
static struct as5916_54xksb_sfp_data *as5916_54xksb_sfp_update_eeprom(int port);
...
struct as5916_54xksb_sfp_data *data = NULL;

static struct platform_driver as5916_54xksb_sfp_driver = {
    .probe      = as5916_54xksb_sfp_probe,
    .remove     = as5916_54xksb_sfp_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
enum as5916_54xksb_qsfp_sysfs_attrs {
    QSFP_ATTR(49),
    QSFP_ATTR(50),
	QSFP_ATTR(51),
    QSFP_ATTR(52),
...
static struct attribute *as5916_54xksb_sfp_attributes[] = {
    /* sfp attributes */
    DECLARE_SFP_ATTR(1),
    DECLARE_SFP_ATTR(2),
    DECLARE_SFP_ATTR(3),
...
static const struct attribute_group as5916_54xksb_sfp_group = {
    .attrs = as5916_54xksb_sfp_attributes,
};
...
static struct as5916_54xksb_sfp_data *as5916_54xksb_qsfp_update_lpmode(void)
{
    int status = 0;
...
static ssize_t show_all(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	switch (attr->index) {
		case PRESENT_ALL:
        {
            data = as5916_54xksb_sfp_update_present();
            if (!data->ipmi_resp.sfp_valid[SFP_PRESENT]) {
                return -EIO;
            }

            data = as5916_54xksb_qsfp_update_present();
		...
		case RXLOS_ALL:
        {
            data = as5916_54xksb_sfp_update_rxlos();
...
static ssize_t show_sfp(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	switch (attr->index) {
        ...
        {
            data = as5916_54xksb_sfp_update_txdisable();
        ...
        case SFP48_TXFAULT:
        {
            data = as5916_54xksb_sfp_update_txfault();
...
static ssize_t show_sfp_eeprom(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = as5916_54xksb_sfp_update_eeprom(pid + 1);
    if (!data->ipmi_resp.eeprom_valid) {
        return -EIO;
    }

    if (!data->ipmi_resp.sfp_resp[SFP_PRESENT][pid]) {
        return -ENXIO;
    }
...
static ssize_t show_qsfp(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	switch (attr->index) {
        ...
        {
            data = as5916_54xksb_qsfp_update_txdisable();
		...
		case QSFP54_RESET:
        {
            data = as5916_54xksb_qsfp_update_reset();
		...
		case QSFP54_LPMODE:
        {
            data = as5916_54xksb_qsfp_update_lpmode();
...
static ssize_t show_qsfp_eeprom(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = as5916_54xksb_sfp_update_eeprom(pid + 1 + NUM_OF_SFP);
    if (!data->ipmi_resp.eeprom_valid) {
        return -EIO;
    }
...
static int as5916_54xksb_sfp_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as5916_54xksb_sfp_group);
	if (status) {
		goto exit;
	}
...
static int as5916_54xksb_sfp_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as5916_54xksb_sfp_group);
    return 0;
}

static int __init as5916_54xksb_sfp_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as5916_54xksb_sfp_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as5916_54xksb_sfp_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&as5916_54xksb_sfp_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as5916_54xksb_sfp_exit(void)
{
...
MODULE_DESCRIPTION("AS5916 54XKSB sfp driver");
MODULE_LICENSE("GPL");

module_init(as5916_54xksb_sfp_init);
module_exit(as5916_54xksb_sfp_exit);
