...
#define DRVNAME "as5916_54xksb_led"
...
static int as5916_54xksb_led_probe(struct platform_device *pdev);
static int as5916_54xksb_led_remove(struct platform_device *pdev);
...
struct as5916_54xksb_led_data *data = NULL;

static struct platform_driver as5916_54xksb_led_driver = {
    .probe      = as5916_54xksb_led_probe,
    .remove     = as5916_54xksb_led_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
static struct attribute *as5916_54xksb_led_attributes[] = {
...
static const struct attribute_group as5916_54xksb_led_group = {
    .attrs = as5916_54xksb_led_attributes,
};
...
static struct as5916_54xksb_led_data *as5916_54xksb_led_update_device(void)
{
    int status = 0;
...
static ssize_t show_led(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = as5916_54xksb_led_update_device();
    if (!data->valid) {
        return -EIO;
    }
...
static int as5916_54xksb_led_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as5916_54xksb_led_group);
	if (status) {
		goto exit;
	}
...
static int as5916_54xksb_led_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as5916_54xksb_led_group);	

    return 0;
}

static int __init as5916_54xksb_led_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as5916_54xksb_led_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as5916_54xksb_led_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&as5916_54xksb_led_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as5916_54xksb_led_exit(void)
{
...
MODULE_DESCRIPTION("AS5916 54XKSB led driver");
MODULE_LICENSE("GPL");

module_init(as5916_54xksb_led_init);
module_exit(as5916_54xksb_led_exit);
