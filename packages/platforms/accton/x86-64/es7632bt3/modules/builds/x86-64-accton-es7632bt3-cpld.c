 ...
 * This includes the:
 *	 Accton es7632bt3 CPLD1/CPLD2/CPLD3
 *
 * Based on:
...
struct es7632bt3_cpld_data {
	enum cpld_type   type;
	struct device   *hwmon_dev;
	struct mutex     update_lock;
};

static const struct i2c_device_id es7632bt3_cpld_id[] = {
	{ "es7632bt3_cpld1", es7632bt3_cpld1 },
	{ "es7632bt3_cpld2", es7632bt3_cpld2 },
	{ "es7632bt3_cpld3", es7632bt3_cpld3 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, es7632bt3_cpld_id);
...
enum es7632bt3_cpld1_sysfs_attributes {
	CPLD_VERSION,
	ACCESS,
	MODULE_PRESENT_ALL,
	MODULE_RXLOS_ALL,
...
static int es7632bt3_cpld_write_internal(struct i2c_client *client, u8 reg,
                                           u8 value);
...
#define DECLARE_TRANSCEIVER_RESET_SENSOR_DEVICE_ATTR(index) \
        static SENSOR_DEVICE_ATTR(module_reset_##index, S_IRUGO | S_IWUSR, \
        			  show_status, set_reset, MODULE_RESET_##index)
...
static struct attribute *es7632bt3_cpld1_attributes[] = {
...
static const struct attribute_group es7632bt3_cpld1_group = {
	.attrs = es7632bt3_cpld1_attributes,
};

static struct attribute *es7632bt3_cpld2_attributes[] = {
	&sensor_dev_attr_version.dev_attr.attr,
	&sensor_dev_attr_access.dev_attr.attr,
	NULL
};

static const struct attribute_group es7632bt3_cpld2_group = {
	.attrs = es7632bt3_cpld2_attributes,
};

static struct attribute *es7632bt3_cpld3_attributes[] = {
	&sensor_dev_attr_version.dev_attr.attr,
	&sensor_dev_attr_access.dev_attr.attr,	
	NULL
};

static const struct attribute_group es7632bt3_cpld3_group = {
	.attrs = es7632bt3_cpld3_attributes,
};
				...
				struct device_attribute *da, char *buf)
{
	...
	struct es7632bt3_cpld_data *data = i2c_get_clientdata(client);

	mutex_lock(&data->update_lock);

	for (i = 0; i < ARRAY_SIZE(regs); i++) {
		status = es7632bt3_cpld_read_internal(client, regs[i]);
	...
	//if (data->type == es7632bt3_cpld2) {
	//   values[3] &= 0xF;
	// }
	//else { /* es7632bt3_cpld3 */
	//values[3] &= 0x3;
	// }
	values[4] &= 0x3;
			      ...
			      char *buf)
{
	...
	status = es7632bt3_cpld_read_internal(client, reg);
			   ...
			   char *buf)
{
	...
	if(attr->index >= MODULE_RESET_1 &&
	   attr->index <= MODULE_RESET_32)
		revert = 1;
			      ...
			      const char *buf, size_t count)
{
	...
	status = es7632bt3_cpld_write_internal(client, reg, status);
	if (unlikely(status < 0))
		goto exit;
		      ...
		      const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = es7632bt3_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0))
		goto exit;
...
static void es7632bt3_cpld_add_client(struct i2c_client *client)
{
...
static void es7632bt3_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int es7632bt3_cpld_probe(struct i2c_client *client,
				   const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct es7632bt3_cpld_data *data;
	int ret = -ENODEV;
	...
	data = kzalloc(sizeof(struct es7632bt3_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
	...
	switch (data->type) {
	case es7632bt3_cpld1:
		group = &es7632bt3_cpld1_group;
		break;
	case es7632bt3_cpld2:
		group = &es7632bt3_cpld2_group;
		break;
	case es7632bt3_cpld3:
		group = &es7632bt3_cpld3_group;
		break;
    	default:
		break;
	}
	...
	es7632bt3_cpld_add_client(client);
	return 0;
...
static int es7632bt3_cpld_remove(struct i2c_client *client)
{
	...
	es7632bt3_cpld_remove_client(client);
...
int es7632bt3_cpld_read(unsigned short cpld_addr, u8 reg)
{
	struct list_head   *list_node = NULL;
	struct cpld_client_node *cpld_node = NULL;
	int ret = -EPERM;
	...
	list_for_each(list_node, &cpld_client_list) {
		...
		if (cpld_node->client->addr == cpld_addr) {
			ret = es7632bt3_cpld_read_internal(cpld_node->client,
							     reg);
			break;
		}
	}
...
EXPORT_SYMBOL(es7632bt3_cpld_read);

int es7632bt3_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
	struct list_head   *list_node = NULL;
	struct cpld_client_node *cpld_node = NULL;
	int ret = -EIO;
	...
	list_for_each(list_node, &cpld_client_list)
	{
		...
		if (cpld_node->client->addr == cpld_addr) {
			ret = es7632bt3_cpld_write_internal(cpld_node->client,
							      reg, value);
			break;
		}
	}
...
EXPORT_SYMBOL(es7632bt3_cpld_write);

static struct i2c_driver es7632bt3_cpld_driver = {
	.driver		= {
		.name	= "es7632bt3_cpld",
		.owner	= THIS_MODULE,
	},
...
static int __init es7632bt3_cpld_init(void)
{
	mutex_init(&list_lock);
	return i2c_add_driver(&es7632bt3_cpld_driver);
}

static void __exit es7632bt3_cpld_exit(void)
{
	i2c_del_driver(&es7632bt3_cpld_driver);
}
...
module_init(es7632bt3_cpld_init);
module_exit(es7632bt3_cpld_exit);
