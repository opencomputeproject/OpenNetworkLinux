/*
 * A hwmon driver for the Accton es7632bt3 32x fan
 *
...
static struct es7632bt3_fan_data *es7632bt3_fan_update_device(struct 
							      device *dev);
...
enum sysfs_fan_attributes {
	FAN_PRESENT_REG,
	FAN_DIRECTION_REG,
	/* Only one CPLD register to control duty cycle for all fans */
...
/* 12 fan fault attributes in this platform */
...
/* 12 fan speed(rpm) attributes in this platform */
...
/* 12 fan present attributes in this platform */
...
DECLARE_FAN_DIRECTION_SENSOR_DEV_ATTR(8);
DECLARE_FAN_DIRECTION_SENSOR_DEV_ATTR(9);
DECLARE_FAN_DIRECTION_SENSOR_DEV_ATTR(10);
DECLARE_FAN_DIRECTION_SENSOR_DEV_ATTR(11);
DECLARE_FAN_DIRECTION_SENSOR_DEV_ATTR(12);
/* 1 fan duty cycle attribute in this platform */
DECLARE_FAN_DUTY_CYCLE_SENSOR_DEV_ATTR();

static struct attribute *es7632bt3_fan_attributes[] = {
	...
	DECLARE_FAN_DIRECTION_ATTR(8),
	DECLARE_FAN_DIRECTION_ATTR(9),
	DECLARE_FAN_DIRECTION_ATTR(10),
	DECLARE_FAN_DIRECTION_ATTR(11),
	DECLARE_FAN_DIRECTION_ATTR(12),
...
static int es7632bt3_fan_read_value(struct i2c_client *client, u8 reg)
{
	return i2c_smbus_read_byte_data(client, reg);
}

static int es7632bt3_fan_write_value(struct i2c_client *client, u8 reg,
					u8 value)
{
...
static u8 is_fan_fault(struct es7632bt3_fan_data *data, enum fan_id id)
{
	u8 ret = 1;
			      ...
			      const char *buf, size_t count)
{
	...
	struct es7632bt3_fan_data *data = i2c_get_clientdata(client);
	 ...
	 */
	error = es7632bt3_fan_write_value(client, 0x33, 0);
	...
	es7632bt3_fan_write_value(client,
			      ...
			      char *buf)
{
	...
	data = es7632bt3_fan_update_device(dev);
	if (data->valid) {
		switch (attr->index) {
		...
		case FAN11_SPEED_RPM:
		case FAN12_SPEED_RPM:
			ret = sprintf(buf, "%u\n", 
...
static const struct attribute_group es7632bt3_fan_group = {
	.attrs = es7632bt3_fan_attributes,
};

static struct es7632bt3_fan_data 
	*es7632bt3_fan_update_device(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct es7632bt3_fan_data *data = i2c_get_clientdata(client);
	...
	if (time_after(jiffies, data->last_updated + HZ + HZ / 2) ||
		...
		dev_dbg(&client->dev, "Starting es7632bt3_fan update\n");
		data->valid = 0;

		/* Update fan data
		 */
		for (i = 0; i < ARRAY_SIZE(data->reg_val); i++) {
			int status = es7632bt3_fan_read_value(client,
							      fan_reg[i]);
			if (status < 0) {
				data->valid = 0;
				mutex_unlock(&data->update_lock);
...
static int es7632bt3_fan_probe(struct i2c_client *client,
				  const struct i2c_device_id *dev_id)
{
	struct es7632bt3_fan_data *data;
	int status;
	...
	data = kzalloc(sizeof(struct es7632bt3_fan_data), GFP_KERNEL);
	if (!data) {
		status = -ENOMEM;
		goto exit;
	}
	...
	status = sysfs_create_group(&client->dev.kobj, &es7632bt3_fan_group);
	if (status)
		goto exit_free;
...
exit_remove:
	sysfs_remove_group(&client->dev.kobj, &es7632bt3_fan_group);
exit_free:
	kfree(data);
exit:
...
static const struct i2c_device_id es7632bt3_fan_id[] = {
	{ "es7632bt3_fan", 0 },
	{}
};
MODULE_DEVICE_TABLE(i2c, es7632bt3_fan_id);

static struct i2c_driver es7632bt3_fan_driver = {
...
module_i2c_driver(es7632bt3_fan_driver);

MODULE_AUTHOR("Alex Lai <alex_lai@edge-core.com>");
MODULE_DESCRIPTION("es7632bt3_fan driver");
MODULE_LICENSE("GPL");
