/*
 * A hwmon driver for the Accton as5916 54xm fan
 *
...
static struct as5916_54xm_fan_data *as5916_54xm_fan_update_device(struct device *dev);                    
...
static struct attribute *as5916_54xm_fan_attributes[] = {
...
static int as5916_54xm_fan_read_value(struct i2c_client *client, u8 reg)
{
    return i2c_smbus_read_byte_data(client, reg);
}

static int as5916_54xm_fan_write_value(struct i2c_client *client, u8 reg, u8 value)
{
...
static u8 is_fan_fault(struct as5916_54xm_fan_data *data, enum fan_id id)
{
    u8 ret = 1;
            ...
            const char *buf, size_t count) 
{
    ...
    as5916_54xm_fan_write_value(client, 0x33, 0); /* Disable fan speed watch dog */
    as5916_54xm_fan_write_value(client, fan_reg[FAN_DUTY_CYCLE_PERCENTAGE], duty_cycle_to_reg_val(value));
    return count;
}
             ...
             char *buf)
{
    ...
    struct as5916_54xm_fan_data *data = as5916_54xm_fan_update_device(dev);
    ssize_t ret = 0;
...
static const struct attribute_group as5916_54xm_fan_group = {
    .attrs = as5916_54xm_fan_attributes,
};

static struct as5916_54xm_fan_data *as5916_54xm_fan_update_device(struct device *dev)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct as5916_54xm_fan_data *data = i2c_get_clientdata(client);
    ...
    if (time_after(jiffies, data->last_updated + HZ + HZ / 2) || 
        ...
        dev_dbg(&client->dev, "Starting as5916_54xm_fan update\n");
        data->valid = 0;
        
        /* Update fan data
         */
        for (i = 0; i < ARRAY_SIZE(data->reg_val); i++) {
            int status = as5916_54xm_fan_read_value(client, fan_reg[i]);
...
static int as5916_54xm_fan_probe(struct i2c_client *client,
            const struct i2c_device_id *dev_id)
{
    struct as5916_54xm_fan_data *data;
    int status;
    ...
    data = kzalloc(sizeof(struct as5916_54xm_fan_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&client->dev.kobj, &as5916_54xm_fan_group);
    if (status) {
        goto exit_free;
    }
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &as5916_54xm_fan_group);
exit_free:
    kfree(data);
exit:
...
static const struct i2c_device_id as5916_54xm_fan_id[] = {
    { "as5916_54xm_fan", 0 },
    {}
};
MODULE_DEVICE_TABLE(i2c, as5916_54xm_fan_id);

static struct i2c_driver as5916_54xm_fan_driver = {
...
static int __init as5916_54xm_fan_init(void)
{
    return i2c_add_driver(&as5916_54xm_fan_driver);
}

static void __exit as5916_54xm_fan_exit(void)
{
    i2c_del_driver(&as5916_54xm_fan_driver);
}

module_init(as5916_54xm_fan_init);
module_exit(as5916_54xm_fan_exit);

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@accton.com.tw>");
MODULE_DESCRIPTION("as5916_54xm_fan driver");
MODULE_LICENSE("GPL");
