...
#define FAN_BOARD_PATH    "/sys/bus/i2c/devices/8-0066/"
...
onlp_fani_info_get(onlp_oid_t id, onlp_fan_info_t* info)
{
    ...
    *info = finfo[fid];
     ...
     */
    sprintf(path, "%s""fantray%d_present", FAN_BOARD_PATH, fid );
    ...
    /* get front fan rpm
     */
    sprintf(path, "%s""fan%d_input", FAN_BOARD_PATH, fid);
...
onlp_fani_percentage_set(onlp_oid_t id, int p)
{
    ...
    sprintf(path, "%d > %sfantray_pwm", p,FAN_BOARD_PATH);
    if (bmc_file_write_int(p, path, 10) < 0) {
        AIM_LOG_ERROR("Unable to read status from file (%s)\r\n", path);
        return ONLP_STATUS_E_INTERNAL;
    }
    return ONLP_STATUS_OK;
