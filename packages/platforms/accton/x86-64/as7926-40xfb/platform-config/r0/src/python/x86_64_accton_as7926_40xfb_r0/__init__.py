...
class OnlPlatform_x86_64_accton_as7926_40xfb_r0(OnlPlatformAccton,
    ...
    def baseconfig(self):
        ...
        for port in range(0, 53):
            ...
            subprocess.call('echo port%d > /sys/bus/i2c/devices/%d-0050/port_name' % (port+1, port_i2c_bus[port]), shell=True)

        # initialize SFP port 54-55
        self.new_i2c_device('optoe2', 0x50, 30)
        subprocess.call('echo port54 > /sys/bus/i2c/devices/30-0050/port_name', shell=True)
        self.new_i2c_device('optoe2', 0x50, 31)
        subprocess.call('echo port55 > /sys/bus/i2c/devices/31-0050/port_name', shell=True)
