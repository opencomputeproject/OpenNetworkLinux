 ...
 * This includes the:
 *	 Accton as5822_54x CPLD1/CPLD2/CPLD3
 *
 * Based on:
...
struct as5822_54x_cpld_data {
    enum cpld_type   type;
    struct device   *hwmon_dev;
    struct mutex     update_lock;
};

static const struct i2c_device_id as5822_54x_cpld_id[] = {
    { "as5822_54x_cpld1", as5822_54x_cpld1 },
    { "as5822_54x_cpld2", as5822_54x_cpld2 },
    { "as5822_54x_cpld3", as5822_54x_cpld3 },
    { }
};
MODULE_DEVICE_TABLE(i2c, as5822_54x_cpld_id);
...
enum as5822_54x_cpld1_sysfs_attributes {
	CPLD_VERSION,
	ACCESS,
	MODULE_PRESENT_ALL,
	MODULE_RXLOS_ALL,
...
static int as5822_54x_cpld_write_internal(struct i2c_client *client, u8 reg, u8 value);
...
static struct attribute *as5822_54x_cpld1_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	NULL
};

static const struct attribute_group as5822_54x_cpld1_group = {
	.attrs = as5822_54x_cpld1_attributes,
};

static struct attribute *as5822_54x_cpld2_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	/* transceiver attributes */
...
static const struct attribute_group as5822_54x_cpld2_group = {
	.attrs = as5822_54x_cpld2_attributes,
};

static struct attribute *as5822_54x_cpld3_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	/* transceiver attributes */
	&sensor_dev_attr_module_present_all.dev_attr.attr,
...
static const struct attribute_group as5822_54x_cpld3_group = {
	.attrs = as5822_54x_cpld3_attributes,
};
             ...
             char *buf)
{
	int i, status;
	u8 values[4]  = {0};
	u8 regs[] = {0x3, 0x4, 0x5, 0x6};
	struct i2c_client *client = to_i2c_client(dev);
	struct as5822_54x_cpld_data *data = i2c_get_clientdata(client);

	mutex_lock(&data->update_lock);

    for (i = 0; i < ARRAY_SIZE(regs); i++) {
        status = as5822_54x_cpld_read_internal(client, regs[i]);
    ...
    if (data->type == as5822_54x_cpld2) {
        values[3] &= 0x1F;
    }
    else { /* as5822_54x_cpld3 */
        values[2] &= 0x7;
        values[3] &= 0x3F;
        values[2] |= (values[3] & 0x1F) << 3;
        values[3] >>= 5; /* shift out port 49-53 */
    }
             ...
             char *buf)
{
	int i, status;
	u8 values[4]  = {0};
    u8 regs_cpld2[] = {0xF, 0x10, 0x11, 0x12};
	u8 regs_cpld3[] = {0xD, 0xE, 0xF};
    ...
    for (i = 0; i < size[data->type]; i++) {
        status = as5822_54x_cpld_read_internal(client, regs[data->type][i]);
        if (status < 0) {
            goto exit;
        }
             ...
             char *buf)
{
	...
	case MODULE_PRESENT_17 ... MODULE_PRESENT_24:
		...
		break;
	case MODULE_PRESENT_25 ... MODULE_PRESENT_29:
		reg  = 0x6;
		mask = 0x1 << (attr->index - MODULE_PRESENT_25);
		break;
	case MODULE_PRESENT_30 ... MODULE_PRESENT_37:
		reg  = 0x3;
		mask = 0x1 << (attr->index - MODULE_PRESENT_30);
		break;
	case MODULE_PRESENT_38 ... MODULE_PRESENT_45:
		reg  = 0x4;
		mask = 0x1 << (attr->index - MODULE_PRESENT_38);
		break;
	case MODULE_PRESENT_46 ... MODULE_PRESENT_48:
		reg  = 0x5;
		mask = 0x1 << (attr->index - MODULE_PRESENT_46);
		break;   
	...
	case MODULE_TXFAULT_17 ... MODULE_TXFAULT_24:
		...
		break;
	case MODULE_TXFAULT_25 ... MODULE_TXFAULT_29:
		reg  = 0xA;
		mask = 0x1 << (attr->index - MODULE_TXFAULT_25);
		break;
	case MODULE_TXFAULT_30 ... MODULE_TXFAULT_37:
		reg  = 0x7;
		mask = 0x1 << (attr->index - MODULE_TXFAULT_30);
		break;
	case MODULE_TXFAULT_38 ... MODULE_TXFAULT_45:
		reg  = 0x8;
		mask = 0x1 << (attr->index - MODULE_TXFAULT_38);
		break;
	case MODULE_TXFAULT_46 ... MODULE_TXFAULT_48:
		reg  = 0x9;
		mask = 0x1 << (attr->index - MODULE_TXFAULT_46);
		break;
	...
	case MODULE_TXDISABLE_17 ... MODULE_TXDISABLE_24:
		...
		break;
	case MODULE_TXDISABLE_25 ... MODULE_TXDISABLE_29:
		reg  = 0xE;
		mask = 0x1 << (attr->index - MODULE_TXDISABLE_25);
		break;
	case MODULE_TXDISABLE_30 ... MODULE_TXDISABLE_37:
		reg  = 0xA;
		mask = 0x1 << (attr->index - MODULE_TXDISABLE_30);
		break;
	case MODULE_TXDISABLE_38 ... MODULE_TXDISABLE_45:
		reg  = 0xB;
		mask = 0x1 << (attr->index - MODULE_TXDISABLE_38);
		break;
	case MODULE_TXDISABLE_46 ... MODULE_TXDISABLE_48:
		reg  = 0xC;
	...
	case MODULE_RXLOS_17 ... MODULE_RXLOS_24:
		...
		break;
	case MODULE_RXLOS_25 ... MODULE_RXLOS_29:
		reg  = 0x12;
		mask = 0x1 << (attr->index - MODULE_RXLOS_25);
		break;
	case MODULE_RXLOS_30 ... MODULE_RXLOS_37:
		reg  = 0xD;
		mask = 0x1 << (attr->index - MODULE_RXLOS_30);
		break;
	case MODULE_RXLOS_38 ... MODULE_RXLOS_45:
		reg  = 0xE;
		mask = 0x1 << (attr->index - MODULE_RXLOS_38);
		break;
	case MODULE_RXLOS_46 ... MODULE_RXLOS_48:
		reg  = 0xF;
    ...
    mutex_lock(&data->update_lock);
	status = as5822_54x_cpld_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
    ...
    status = as5822_54x_cpld_write_internal(client, reg, status);
	if (unlikely(status < 0)) {
		goto exit;
	}
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = as5822_54x_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0)) {
		goto exit;
	}
	mutex_unlock(&data->update_lock);
...
static void as5822_54x_cpld_add_client(struct i2c_client *client)
{
...
static void as5822_54x_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int as5822_54x_cpld_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct as5822_54x_cpld_data *data;
	int ret = -ENODEV;
	...
	data = kzalloc(sizeof(struct as5822_54x_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
    ...
    switch (data->type) {
    case as5822_54x_cpld1:
        group = &as5822_54x_cpld1_group;
        break;
    case as5822_54x_cpld2:
        group = &as5822_54x_cpld2_group;
        break;
	case as5822_54x_cpld3:
        group = &as5822_54x_cpld3_group;
        break;
    default:
        break;
    }
    ...
    as5822_54x_cpld_add_client(client);
    return 0;
...
static int as5822_54x_cpld_remove(struct i2c_client *client)
{
    ...
    as5822_54x_cpld_remove_client(client);
...
int as5822_54x_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as5822_54x_cpld_read_internal(cpld_node->client, reg);
    		break;
        }
    }
...
EXPORT_SYMBOL(as5822_54x_cpld_read);

int as5822_54x_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as5822_54x_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
...
EXPORT_SYMBOL(as5822_54x_cpld_write);

static struct i2c_driver as5822_54x_cpld_driver = {
	.driver		= {
...
static int __init as5822_54x_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&as5822_54x_cpld_driver);
}

static void __exit as5822_54x_cpld_exit(void)
{
    i2c_del_driver(&as5822_54x_cpld_driver);
}
...
module_init(as5822_54x_cpld_init);
module_exit(as5822_54x_cpld_exit);
