...
MODULE_PARM_DESC(timeout,
		 "Tiemout for ocores_poll_wait, in unit of milliseconds.");

static unsigned int debug = 0;
module_param(debug, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(debug,
		 "Enable or disable debug message. 1 -> enable, 0 -> disable");
...
static void ocores_process(struct ocores_i2c *i2c, u8 stat)
{
	...
	if (stat & OCI2C_STAT_ARBLOST) {
		...
		if (debug) {
			dev_warn(dev, "I2C %s arbitration lost",
				 i2c->adap.name);
		}
	...
	if ((i2c->state == STATE_START) || (i2c->state == STATE_WRITE)) {
		...
		if (stat & OCI2C_STAT_NACK) {
			...
			if (debug) {
				dev_warn(dev,
					 "I2C %s, no ACK from slave 0x%02x",
...
static int ocores_wait(struct ocores_i2c *i2c,
		       int reg, u8 mask, u8 val, const unsigned long timeout)
{
	unsigned long j;
...
static int ocores_i2c_probe(struct platform_device *pdev)
{
	...
	if (res) {
		...
		dev_info(&pdev->dev, "Resouce start:0x%llx, end:0x%llx",
			 res->start, res->end);
		if (IS_ERR(i2c->base))
	...
	if (pdata) {
		...
		dev_info(&pdev->dev, "Write %d KHz, ioWidth:%d, shift:%d",
			 i2c->ip_clock_khz, pdata->reg_io_width,
			 pdata->reg_shift);
		i2c->bus_clock_khz = 100;
	} else {
