 ...
 * This includes the:
 *	 Accton as7315_30x fpga1/fpga2/fpga3
 *
 * Based on:
...
static const struct i2c_device_id as7315_30x_fpga_mux_id[] = {
	{ "as7315_30x_fpga", as7315_30x_fpga },
	{ }
};
MODULE_DEVICE_TABLE(i2c, as7315_30x_fpga_mux_id);

#define TRANSCEIVER_PRESENT_ATTR_ID(index) MODULE_PRESENT_##index
#define TRANSCEIVER_LPMODE_ATTR_ID(index)  MODULE_LPMODE_##index
#define TRANSCEIVER_RESET_ATTR_ID(index)  MODULE_RESET_##index

enum as7315_30x_fpga_sysfs_attributes {
	FPGA_VERSION,
	PCB_VERSION,
	ACCESS,
	MODULE_PRESENT_ALL,
...
static int as7315_30x_fpga_write_internal(struct i2c_client *client, u8 reg,
			u8 value);
...
static struct attribute *as7315_30x_fpga_attributes[] = {
...
static const struct attribute_group as7315_30x_fpga_group = {
	.attrs = as7315_30x_fpga_attributes,
};
			...
			char *buf)
{
	...
	u8 regs_fpga[] = { 0x18 };
	u8 *regs[] = { regs_fpga };
	u8  size[] = { ARRAY_SIZE(regs_fpga) };
	struct i2c_client *client = to_i2c_client(dev);
	struct i2c_mux_core *muxc = i2c_get_clientdata(client);
	struct as7315_30x_fpga_data *data = i2c_mux_priv(muxc);

	mutex_lock(&data->update_lock);

	for (i = 0; i < size[data->type]; i++) {
		status = as7315_30x_fpga_read_internal(client, regs[data->type][i]);
		if (status < 0) {
			goto exit;
		}
			 ...
			 char *buf)
{
	...
	case MODULE_RESET_25 ... MODULE_RESET_26:
		invert = 1;
		reg  = 0x31;
		mask = 0x1 << (attr->index - MODULE_RESET_25 + 4);
		break;
	default:
		return 0;
	}
	...
	mutex_lock(&data->update_lock);
	status = as7315_30x_fpga_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
	...
	case MODULE_RESET_25 ... MODULE_RESET_26:
		invert = 1;
		reg  = 0x31;
		mask = 0x10 << (attr->index - MODULE_RESET_25);
		break;
	default:
		return 0;
	}
	...
	status = as7315_30x_fpga_write_internal(client, reg, status);
	if (unlikely(status < 0))
		goto exit;
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = as7315_30x_fpga_write_internal(client, addr, val);
	if (unlikely(status < 0))
		goto exit;
	mutex_unlock(&data->update_lock);
	return count;
...
static int as7315_30x_fpga_mux_reg_write(struct i2c_adapter *adap,
...
static int as7315_30x_fpga_mux_select_chan(struct i2c_mux_core *muxc,
	u32 chan)
{
	struct as7315_30x_fpga_data *data = i2c_mux_priv(muxc);
	...
	if (data->last_chan != regval) {
		ret = as7315_30x_fpga_mux_reg_write(muxc->parent, client, regval);
...
static int as7315_30x_fpga_mux_deselect_mux(struct i2c_mux_core *muxc,
	u32 chan)
{
	struct as7315_30x_fpga_data *data = i2c_mux_priv(muxc);
	struct i2c_client *client = data->client;
	...
	return as7315_30x_fpga_mux_reg_write(muxc->parent, client, data->last_chan);
}

static void as7315_30x_fpga_add_client(struct i2c_client *client)
{
...
static void as7315_30x_fpga_remove_client(struct i2c_client *client)
{
 ...
 */
static int as7315_30x_fpga_mux_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	int num, force, class;
	struct i2c_mux_core *muxc;
	struct as7315_30x_fpga_data *data;
	int ret = 0;
	const struct attribute_group *group = &as7315_30x_fpga_group;
	...
	muxc = i2c_mux_alloc(adap, &client->dev,
				...
				as7315_30x_fpga_mux_select_chan,
				as7315_30x_fpga_mux_deselect_mux);
	if (!muxc)
		return -ENOMEM;
	...
	as7315_30x_fpga_add_client(client);
...
static int as7315_30x_fpga_mux_remove(struct i2c_client *client)
{
	...
	as7315_30x_fpga_remove_client(client);
...
static int as7315_30x_fpga_write_internal(struct i2c_client *client, u8 reg, u8 value)
{
	int status = 0, retry = I2C_RW_RETRY_COUNT;
...
int as7315_30x_fpga_read(unsigned short fpga_addr, u8 reg)
{
	struct list_head   *list_node = NULL;
	struct fpga_client_node *fpga_node = NULL;
	int ret = -EPERM;
	...
	list_for_each(list_node, &fpga_client_list) {
		...
		if (fpga_node->client->addr == fpga_addr) {
			ret = as7315_30x_fpga_read_internal(fpga_node->client, reg);
			break;
		}
	}
...
EXPORT_SYMBOL(as7315_30x_fpga_read);

int as7315_30x_fpga_write(unsigned short fpga_addr, u8 reg, u8 value)
{
	struct list_head   *list_node = NULL;
	struct fpga_client_node *fpga_node = NULL;
	int ret = -EIO;
	...
	list_for_each(list_node, &fpga_client_list) {
		...
		if (fpga_node->client->addr == fpga_addr) {
			ret = as7315_30x_fpga_write_internal(fpga_node->client, reg, value);
			break;
		}
	}
...
EXPORT_SYMBOL(as7315_30x_fpga_write);

static struct i2c_driver as7315_30x_fpga_mux_driver = {
	.driver		= {
...
static int __init as7315_30x_fpga_mux_init(void)
{
	mutex_init(&list_lock);
	return i2c_add_driver(&as7315_30x_fpga_mux_driver);
}

static void __exit as7315_30x_fpga_mux_exit(void)
{
	i2c_del_driver(&as7315_30x_fpga_mux_driver);
}

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@edge-core.com>");
MODULE_DESCRIPTION("as7315_30x_fpga driver");
MODULE_LICENSE("GPL");

module_init(as7315_30x_fpga_mux_init);
module_exit(as7315_30x_fpga_mux_exit);
