 ...
 * This includes the:
 *	 Accton as7315_30x CPLD1/CPLD2/CPLD3
 *
 * Based on:
...
struct as7315_30x_cpld_data {
	enum cpld_type type;
	struct device *hwmon_dev;
	struct mutex update_lock;
};

static const struct i2c_device_id as7315_30x_cpld_id[] = {
	{ "as7315_30x_cpld", as7315_30x_cpld },
	{ }
};
MODULE_DEVICE_TABLE(i2c, as7315_30x_cpld_id);
...
enum as7315_30x_cpld_sysfs_attributes {
	CPLD_VERSION,
	CPLD_SUB_VERSION,
	ACCESS,
	MODULE_PRESENT_ALL,
...
static int as7315_30x_cpld_write_internal(struct i2c_client *client, u8 reg,
			u8 value);
...
#define DECLARE_SFP_TRANSCEIVER_SENSOR_DEVICE_ATTR(index) \
	...
	static SENSOR_DEVICE_ATTR(module_tx_disable_##index, S_IRUGO | S_IWUSR, \
						show_status, set_control, MODULE_TXDISABLE_##index); \
...
static struct attribute *as7315_30x_cpld_attributes[] = {
...
static const struct attribute_group as7315_30x_cpld_group = {
	.attrs = as7315_30x_cpld_attributes,
};
			...
			char *buf)
{
	int i, status;
	u8 values[3]  = { 0 };
	u8 regs_cpld[] = { 0x10, 0x14, 0x18 };
	u8 *regs[] = { regs_cpld };
	u8  size[] = { ARRAY_SIZE(regs_cpld) };
	struct i2c_client *client = to_i2c_client(dev);
	struct as7315_30x_cpld_data *data = i2c_get_clientdata(client);

	mutex_lock(&data->update_lock);

	for (i = 0; i < size[data->type]; i++) {
		status = as7315_30x_cpld_read_internal(client, regs[data->type][i]);
		if (status < 0) {
			goto exit;
		}
			 ...
			 char *buf)
{
	...
	mutex_lock(&data->update_lock);
	status = as7315_30x_cpld_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
	...
	status = as7315_30x_cpld_write_internal(client, reg, status);
	if (unlikely(status < 0))
		goto exit;
			...
			const char *buf, size_t count)
{
	...
	status = as7315_30x_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0))
		goto exit;
...
static void as7315_30x_cpld_add_client(struct i2c_client *client)
{
...
static void as7315_30x_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int as7315_30x_cpld_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct as7315_30x_cpld_data *data;
	int ret = -ENODEV;
	const struct attribute_group *group = &as7315_30x_cpld_group;

	if (!i2c_check_functionality(adap, I2C_FUNC_SMBUS_BYTE))
		goto exit;

	data = kzalloc(sizeof(struct as7315_30x_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
	...
	as7315_30x_cpld_add_client(client);
	return 0;
...
static int as7315_30x_cpld_remove(struct i2c_client *client)
{
	...
	as7315_30x_cpld_remove_client(client);
...
int as7315_30x_cpld_read(unsigned short cpld_addr, u8 reg)
{
	struct list_head   *list_node = NULL;
	struct cpld_client_node *cpld_node = NULL;
	int ret = -EPERM;
	...
	list_for_each(list_node, &cpld_client_list)
	{
		...
		if (cpld_node->client->addr == cpld_addr) {
			ret = as7315_30x_cpld_read_internal(cpld_node->client, reg);
			break;
		}
	}
...
EXPORT_SYMBOL(as7315_30x_cpld_read);

int as7315_30x_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
	struct list_head   *list_node = NULL;
	struct cpld_client_node *cpld_node = NULL;
	int ret = -EIO;
	...
	list_for_each(list_node, &cpld_client_list)
	{
		...
		if (cpld_node->client->addr == cpld_addr) {
			ret = as7315_30x_cpld_write_internal(cpld_node->client, reg, value);
			break;
		}
	}
...
EXPORT_SYMBOL(as7315_30x_cpld_write);

static struct i2c_driver as7315_30x_cpld_driver = {
	.driver = {
...
static int __init as7315_30x_cpld_init(void)
{
	mutex_init(&list_lock);
	return i2c_add_driver(&as7315_30x_cpld_driver);
}

static void __exit as7315_30x_cpld_exit(void)
{
	i2c_del_driver(&as7315_30x_cpld_driver);
}

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@accton.com.tw>");
MODULE_DESCRIPTION("as7315_30x_cpld driver");
MODULE_LICENSE("GPL");

module_init(as7315_30x_cpld_init);
module_exit(as7315_30x_cpld_exit);
