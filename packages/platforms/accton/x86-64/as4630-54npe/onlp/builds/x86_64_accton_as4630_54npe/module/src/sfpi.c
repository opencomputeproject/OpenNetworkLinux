...
onlp_sfpi_presence_bitmap_get(onlp_sfp_bitmap_t* dst)
{
    ...
    uint32_t present_bytes[6] = {0};


    for(port = 48; port < 54; port++) {
        present_bytes[port - 48] = onlp_sfpi_is_present(port);
    }
    /* Convert to 64 bit integer in port order */
    for (i = 0 ; i < 6 ;i++)
        presence_all |= (present_bytes[i] << i);
    presence_all <<= 48;
    /* Populate bitmap */
...
onlp_sfpi_rx_los_bitmap_get(onlp_sfp_bitmap_t* dst)
{
    ...
    int addr = 60;
    int bus  = 3;
    int port, rxlos_bytes[4] = {0};

    for(port = 49; port < 53 ; port++)
    {
        if (onlp_file_read_int(&rxlos_bytes[port-49], MODULE_RXLOS_FORMAT, bus, addr, port) < 0)
        {
    ...
    /* Convert to 64 bit integer in port order */
    for (i = 0; i < 4 ; i++)
        rx_los_all = (rx_los_all | ((rxlos_bytes[i]) << i));
    rx_los_all = (rx_los_all << 48);
    /* Populate bitmap */
    for(i = 0; i < 64; i++) {
