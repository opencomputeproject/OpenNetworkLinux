...
_onlp_fani_info_get_fan(int fid, onlp_fan_info_t* info)
{
	 ...
	 */
	sprintf(path, "%s""fan_present_%d", CPLD_NODE_PATH, fid);
     ...
     */
	sprintf(path, "%s""fan_fault_%d", CPLD_NODE_PATH, fid);
     ...
     */
	sprintf(path, "%s""fan_direction_%d", CPLD_NODE_PATH, fid);
    ...
    /* get front fan speed
     */
	sprintf(path, "%s""fan_speed_rpm_%d", CPLD_NODE_PATH, fid);
	...
	sprintf(path, "%s""fan_duty_cycle_percentage", CPLD_NODE_PATH);
	DEBUG_PRINT("Fan (%d), duty_cycle path = (%s)", fid, path);
    if (onlp_file_read_int(&value, path) < 0) {
        AIM_LOG_ERROR("Unable to read status from file (%s)\r\n", path);
        return ONLP_STATUS_E_INTERNAL;
    } 
