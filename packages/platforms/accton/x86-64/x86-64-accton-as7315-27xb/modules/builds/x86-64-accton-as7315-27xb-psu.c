...
#define DRVNAME "as7516_27xb_psu"
...
static int as7516_27xb_psu_probe(struct platform_device *pdev);
static int as7516_27xb_psu_remove(struct platform_device *pdev);
...
struct as7516_27xb_psu_data *data = NULL;

static struct platform_driver as7516_27xb_psu_driver = {
    .probe      = as7516_27xb_psu_probe,
    .remove     = as7516_27xb_psu_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
enum as7516_54x_psu_sysfs_attrs {
	/* psu attributes */
    PSU_ATTR(1),
    PSU_ATTR(2),
    NUM_OF_PSU_ATTR,
...
static struct attribute *as7516_27xb_psu_attributes[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(1),
    DECLARE_PSU_ATTR(2),
    NULL
...
static const struct attribute_group as7516_27xb_psu_group = {
    .attrs = as7516_27xb_psu_attributes,
};
...
static struct as7516_27xb_psu_data *as7516_27xb_psu_update_device(struct device_attribute *da)
{
...
static ssize_t show_psu(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = as7516_27xb_psu_update_device(da);
    if (!data->valid[pid]) {
        return -EIO;
    }
...
static ssize_t show_string(struct device *dev, struct device_attribute *da, char *buf)
{
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    unsigned char pid = attr->index / NUM_OF_PER_PSU_ATTR;
    struct as7516_27xb_psu_data *data;
    char *str = NULL;
...
static int as7516_27xb_psu_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as7516_27xb_psu_group);
	if (status) {
		goto exit;
	}
...
static int as7516_27xb_psu_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as7516_27xb_psu_group);
    return 0;
}

static int __init as7516_27xb_psu_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as7516_27xb_psu_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as7516_27xb_psu_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&as7516_27xb_psu_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as7516_27xb_psu_exit(void)
{
...
MODULE_DESCRIPTION("AS7516 27XB PSU driver");
MODULE_LICENSE("GPL");

module_init(as7516_27xb_psu_init);
module_exit(as7516_27xb_psu_exit);
