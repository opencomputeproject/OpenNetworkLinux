 ...
 * This includes the:
 *	 Accton as7315_27xb CPLD1/CPLD2
 *
 * Based on:
...
struct as7315_27xb_cpld_data {
    enum cpld_type   type;
    struct device   *hwmon_dev;
    struct mutex     update_lock;
};

static const struct i2c_device_id as7315_27xb_cpld_id[] = {
    { "as7315_27xb_cpld1", as7315_27xb_cpld1 },
    { "as7315_27xb_cpld2", as7315_27xb_cpld2 },
    { }
};
MODULE_DEVICE_TABLE(i2c, as7315_27xb_cpld_id);
...
enum as7315_27xb_cpld1_sysfs_attributes {
	CPLD_VERSION,
	ACCESS,
	MODULE_PRESENT_ALL,
	MODULE_RXLOS_ALL,
...
static ssize_t show_version(struct device *dev, struct device_attribute *da,
             char *buf);
static int as7315_27xb_cpld_read_internal(struct i2c_client *client, u8 reg);
static int as7315_27xb_cpld_write_internal(struct i2c_client *client, u8 reg, u8 value);
...
static struct attribute *as7315_27xb_cpld1_attributes[] = {
...
static const struct attribute_group as7315_27xb_cpld1_group = {
	.attrs = as7315_27xb_cpld1_attributes,
};

static struct attribute *as7315_27xb_cpld2_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	/* transceiver attributes */
	&sensor_dev_attr_module_present_all.dev_attr.attr,
...
static const struct attribute_group as7315_27xb_cpld2_group = {
	.attrs = as7315_27xb_cpld2_attributes,
};
             ...
             char *buf)
{
	...
	struct as7315_27xb_cpld_data *data = i2c_get_clientdata(client);

	mutex_lock(&data->update_lock);

    num_regs = (data->type == as7315_27xb_cpld1) ? 3 : 4;

    for (i = 0; i < num_regs; i++) {
        status = as7315_27xb_cpld_read_internal(client, regs[i]);
    ...
    if (data->type == as7315_27xb_cpld1) {
        status = sprintf(buf, "%.2x %.2x %.2x\n",
                              values[0], values[1], values[2]);
    }
    else { /* as7315_27xb_cpld2 */
        values[3] &= 0x3F;
             ...
             char *buf)
{
    ...
    mutex_lock(&data->update_lock);
	status = as7315_27xb_cpld_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
    ...
    status = as7315_27xb_cpld_write_internal(client, reg, status);
	if (unlikely(status < 0)) {
		goto exit;
	}
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = as7315_27xb_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0)) {
		goto exit;
	}
	mutex_unlock(&data->update_lock);
...
static void as7315_27xb_cpld_add_client(struct i2c_client *client)
{
...
static void as7315_27xb_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int as7315_27xb_cpld_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct as7315_27xb_cpld_data *data;
	int ret = -ENODEV;
	...
	data = kzalloc(sizeof(struct as7315_27xb_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
    ...
    switch (data->type) {
    case as7315_27xb_cpld1:
        group = &as7315_27xb_cpld1_group;
        break;
    case as7315_27xb_cpld2:
        group = &as7315_27xb_cpld2_group;
        break;
    default:
        break;
    }
    ...
    as7315_27xb_cpld_add_client(client);
    return 0;
...
static int as7315_27xb_cpld_remove(struct i2c_client *client)
{
    struct as7315_27xb_cpld_data *data = i2c_get_clientdata(client);
    const struct attribute_group *group = NULL;

    as7315_27xb_cpld_remove_client(client);
...
int as7315_27xb_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as7315_27xb_cpld_read_internal(cpld_node->client, reg);
    		break;
        }
    }
...
EXPORT_SYMBOL(as7315_27xb_cpld_read);

int as7315_27xb_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as7315_27xb_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
...
EXPORT_SYMBOL(as7315_27xb_cpld_write);

static struct i2c_driver as7315_27xb_cpld_driver = {
	.driver		= {
		.name	= "as7315_27xb_cpld",
		.owner	= THIS_MODULE,
	},
	.probe		= as7315_27xb_cpld_probe,
	.remove		= as7315_27xb_cpld_remove,
	.id_table	= as7315_27xb_cpld_id,
};

static int __init as7315_27xb_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&as7315_27xb_cpld_driver);
}

static void __exit as7315_27xb_cpld_exit(void)
{
    i2c_del_driver(&as7315_27xb_cpld_driver);
}
...
module_init(as7315_27xb_cpld_init);
module_exit(as7315_27xb_cpld_exit);
