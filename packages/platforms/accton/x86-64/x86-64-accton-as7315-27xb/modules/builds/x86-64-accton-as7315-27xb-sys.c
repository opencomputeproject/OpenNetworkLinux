...
#define DRVNAME "as7516_27xb_sys"
...
static int as7516_27xb_sys_probe(struct platform_device *pdev);
static int as7516_27xb_sys_remove(struct platform_device *pdev);
...
struct as7516_27xb_sys_data *data = NULL;

static struct platform_driver as7516_27xb_sys_driver = {
    .probe      = as7516_27xb_sys_probe,
    .remove     = as7516_27xb_sys_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
static int __init as7516_27xb_sys_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as7516_27xb_sys_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as7516_27xb_sys_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&as7516_27xb_sys_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as7516_27xb_sys_exit(void)
{
...
MODULE_DESCRIPTION("AS7516 27XB System driver");
MODULE_LICENSE("GPL");

module_init(as7516_27xb_sys_init);
module_exit(as7516_27xb_sys_exit);
