...
#define DRVNAME "as7516_27xb_sfp"
...
static int as7516_27xb_sfp_probe(struct platform_device *pdev);
static int as7516_27xb_sfp_remove(struct platform_device *pdev);
static ssize_t show_all(struct device *dev, struct device_attribute *da, char *buf);
static struct as7516_27xb_sfp_data *as7516_27xb_sfp_update_present(void);
static struct as7516_27xb_sfp_data *as7516_27xb_sfp_update_txdisable(void);
static struct as7516_27xb_sfp_data *as7516_27xb_sfp_update_txfault(void);
static struct as7516_27xb_sfp_data *as7516_27xb_sfp_update_rxlos(void);
static struct as7516_27xb_sfp_data *as7516_27xb_qsfp_update_present(void);
static struct as7516_27xb_sfp_data *as7516_27xb_qsfp_update_txdisable(void);
static struct as7516_27xb_sfp_data *as7516_27xb_qsfp_update_reset(void);
static struct as7516_27xb_sfp_data *as7516_27xb_sfp_update_eeprom(int port);
...
struct as7516_27xb_sfp_data *data = NULL;

static struct platform_driver as7516_27xb_sfp_driver = {
    .probe      = as7516_27xb_sfp_probe,
    .remove     = as7516_27xb_sfp_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
enum as7516_54x_sfp_sysfs_attrs {
	SFP_ATTR(1),
    SFP_ATTR(2),
    SFP_ATTR(3),
    SFP_ATTR(4),
...
enum as7516_54x_qsfp_sysfs_attrs {
    QSFP_ATTR(25),
    QSFP_ATTR(26),
    NUM_OF_QSFP_ATTR,
...
static struct attribute *as7516_27xb_sfp_attributes[] = {
    /* sfp attributes */
    DECLARE_SFP_ATTR(1),
    DECLARE_SFP_ATTR(2),
    DECLARE_SFP_ATTR(3),
...
static const struct attribute_group as7516_27xb_sfp_group = {
    .attrs = as7516_27xb_sfp_attributes,
};
...
static ssize_t show_all(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	switch (attr->index) {
		case PRESENT_ALL:
        {
            data = as7516_27xb_sfp_update_present();
            if (!data->ipmi_resp.sfp_valid[SFP_PRESENT]) {
                return -EIO;
            }

            data = as7516_27xb_qsfp_update_present();
		...
		case RXLOS_ALL:
        {
            data = as7516_27xb_sfp_update_rxlos();
...
static ssize_t show_sfp(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	switch (attr->index) {
        ...
        {
            data = as7516_27xb_sfp_update_txdisable();
        ...
        case SFP24_TXFAULT:
        {
            data = as7516_27xb_sfp_update_txfault();
...
static ssize_t show_sfp_eeprom(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = as7516_27xb_sfp_update_eeprom(pid + 1);
...
static ssize_t show_qsfp(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	switch (attr->index) {
        ...
        {
            data = as7516_27xb_qsfp_update_txdisable();
		...
		case QSFP26_RESET:
        {
            data = as7516_27xb_qsfp_update_reset();
...
static ssize_t show_qsfp_eeprom(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = as7516_27xb_sfp_update_eeprom(pid + 1 + NUM_OF_SFP);
    if (!data->ipmi_resp.eeprom_valid) {
        return -EIO;
    }
...
static int as7516_27xb_sfp_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as7516_27xb_sfp_group);
	if (status) {
		goto exit;
	}
...
static int as7516_27xb_sfp_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as7516_27xb_sfp_group);
    return 0;
}

static int __init as7516_27xb_sfp_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as7516_27xb_sfp_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as7516_27xb_sfp_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&as7516_27xb_sfp_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as7516_27xb_sfp_exit(void)
{
...
MODULE_DESCRIPTION("AS7516 27XB sfp driver");
MODULE_LICENSE("GPL");

module_init(as7516_27xb_sfp_init);
module_exit(as7516_27xb_sfp_exit);
