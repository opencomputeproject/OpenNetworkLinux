...
_onlp_fani_info_get_fan(int fid, onlp_fan_info_t* info)
{
     ...
     */
    if (onlp_file_read_int(&value, "%s""fan_present_%d", CPLD_NODE_PATH, fid) < 0) {
        AIM_LOG_ERROR("Unable to read fan presence status from CPLD(%s)\r\n", CPLD_NODE_PATH);
     ...
     */
    if (onlp_file_read_int(&value, "%s""fan_fault_%d", CPLD_NODE_PATH, fid) < 0) {
        AIM_LOG_ERROR("Unable to read fan fault status from CPLD(%s)\r\n", CPLD_NODE_PATH);
        return ONLP_STATUS_E_INTERNAL;
    }
     ...
     */
    if (onlp_file_read_int(&value, "%s""fan_direction_%d", CPLD_NODE_PATH, fid) < 0) {
        AIM_LOG_ERROR("Unable to read fan direction from CPLD(%s)\r\n", CPLD_NODE_PATH);
        return ONLP_STATUS_E_INTERNAL;
    }
    ...
    /* get front fan speed
     */
    if (onlp_file_read_int(&value, "%s""fan_speed_rpm_%d", CPLD_NODE_PATH, fid) < 0) {
        AIM_LOG_ERROR("Unable to read fan speed from CPLD(%s)\r\n", CPLD_NODE_PATH);
        return ONLP_STATUS_E_INTERNAL;
    }
    info->rpm = value;
    ...
    /* get pwm percentage
     */
    if (onlp_file_read_int(&value, "%s""fan_duty_cycle_percentage", CPLD_NODE_PATH) < 0) {
        AIM_LOG_ERROR("Unable to read fan duty from CPLD(%s)\r\n", CPLD_NODE_PATH);
        return ONLP_STATUS_E_INTERNAL;
    }
...
_onlp_get_fan_direction_on_psu(void)
{
    ...
    for (i = PSU1_ID; i <= PSU2_ID; i++) {
        ...
        if (PSU_TYPE_YM1151D_F2B == psu_type ||
            ...
            PSU_TYPE_UPD1501SA_1190G_F2B == psu_type) {
