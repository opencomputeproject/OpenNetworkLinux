...
/*fan sysfs*/
static struct as4630_54te_cpld_data *as4630_54te_fan_update_device(
    						struct device *dev);
...
#define DECLARE_FAN_SENSOR_DEV_ATTR(index) \
	static SENSOR_DEVICE_ATTR(fan_present_##index, S_IRUGO, fan_show_value, \
				NULL, FAN_PRESENT_##index); \
	static SENSOR_DEVICE_ATTR(fan_fault_##index,   S_IRUGO, fan_show_value, \
				NULL, FAN_FAULT_##index); \
	static SENSOR_DEVICE_ATTR(fan_speed_rpm_##index, S_IRUGO, fan_show_value, \
				NULL, FAN_SPEED_RPM_##index); \
	static SENSOR_DEVICE_ATTR(fan_direction_##index, S_IRUGO, fan_show_value, \
				NULL, FAN_DIRECTION_##index);
...
#define DECLARE_FAN_DUTY_CYCLE_SENSOR_DEV_ATTR(index) \
	static SENSOR_DEVICE_ATTR(fan_duty_cycle_percentage, S_IWUSR | S_IRUGO, \
				fan_show_value, set_duty_cycle, FAN_DUTY_CYCLE_PERCENTAGE);
			...
			char *buf)
{
	...
	for (i = 0; i < size[data->type]; i++) {
		status = as4630_54te_cpld_read_internal(client, regs[data->type][i]);
		if (status < 0) {
			goto exit;
		}
	...
	value_ordered = ((values[0] & BIT(6)) >> 6) | ((values[0] & BIT(2)) >> 1) |
					((values[1] & BIT(6)) >> 4) | ((values[1] & BIT(2)) << 1) |
					((values[2] & BIT(0)) << 4) | ((values[2] & BIT(4)) << 1);
			 ...
			 char *buf)
{
	...
	value_ordered = ((values[0] & BIT(4)) >> 4) | ((values[0] & BIT(0)) << 1) |
					((values[1] & BIT(4)) >> 2) | ((values[1] & BIT(0)) << 3);
							  ...
							  const char *buf, size_t count)
{
	...
	reg_val = duty_cycle_to_reg_val(value);

	mutex_lock(&data->update_lock);

	as4630_54te_cpld_write_internal(client, fan_reg[1], reg_val);
	as4630_54te_cpld_write_internal(client, fan_reg[2], reg_val);
	data->valid = 0;
							  ...
							  char *buf)
{
	...
	data = as4630_54te_fan_update_device(dev);
	if (!data->valid) {
		ret = -EIO;
		goto exit;
	}
...
static int as4630_54te_cpld_write_internal(struct i2c_client *client, u8 reg,
										u8 value)
{
	int status = 0, retry = I2C_RW_RETRY_COUNT;
