...
static int convert_led_light_mode_to_onl(int led_ligth_mode)
{
...
static int convert_led_light_mode_to_driver(int led_ligth_mode)
{
    int i, nsize = sizeof(led_light_map_mode)/sizeof(led_light_map_mode[0]);
    for(i=0; i<nsize; i++)
    {
...
onlp_ledi_info_get(onlp_oid_t id, onlp_led_info_t* info)
{
    ...
    *info = linfo[ONLP_OID_ID_GET(id)];
    ...
    info->mode = convert_led_light_mode_to_onl(atoi(data));
