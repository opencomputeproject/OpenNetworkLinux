...
#define DRVNAME "as9947_36xkb_led"
...
static ssize_t show_led(struct device *dev, struct device_attribute *attr,
            char *buf);
static int as9947_36xkb_led_probe(struct platform_device *pdev);
static int as9947_36xkb_led_remove(struct platform_device *pdev);
...
struct as9947_36xkb_led_data *data = NULL;

static struct platform_driver as9947_36xkb_led_driver = {
    .probe = as9947_36xkb_led_probe,
    .remove = as9947_36xkb_led_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
enum as9947_36xkb_led_sysfs_attrs {
    LED_LOC,
    LED_DIAG,
    LED_FAN,
    LED_PSU1,
...
static struct attribute *as9947_36xkb_led_attributes[] = {
...
static const struct attribute_group as9947_36xkb_led_group = {
    .attrs = as9947_36xkb_led_attributes,
};
...
static struct as9947_36xkb_led_data *as9947_36xkb_led_update_device(void)
{
    int status = 0;
                            ...
                            char *buf)
{
    ...
    data = as9947_36xkb_led_update_device();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static int as9947_36xkb_led_probe(struct platform_device *pdev)
{
    ...
    status = sysfs_create_group(&pdev->dev.kobj, &as9947_36xkb_led_group);
    if (status)
        goto exit;
...
static int as9947_36xkb_led_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as9947_36xkb_led_group);

    return 0;
}

static int __init as9947_36xkb_led_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as9947_36xkb_led_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as9947_36xkb_led_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&as9947_36xkb_led_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as9947_36xkb_led_exit(void)
{
...
MODULE_DESCRIPTION("as9947_36xkb_led driver");
MODULE_LICENSE("GPL");

module_init(as9947_36xkb_led_init);
module_exit(as9947_36xkb_led_exit);
