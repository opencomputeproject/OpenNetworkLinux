...
#define DRVNAME "as9947_36xkb_fan"
...
static ssize_t show_threshold(struct device *dev, struct device_attribute *da,
            char *buf);
static int as9947_36xkb_fan_probe(struct platform_device *pdev);
static int as9947_36xkb_fan_remove(struct platform_device *pdev);
...
struct as9947_36xkb_fan_data *data = NULL;

static struct platform_driver as9947_36xkb_fan_driver = {
    .probe = as9947_36xkb_fan_probe,
    .remove = as9947_36xkb_fan_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
enum as9947_36xkb_fan_sysfs_attrs {
    FAN_ATTR(1),
    FAN_ATTR(2),
    FAN_ATTR(3),
    FAN_ATTR(4),
...
static struct attribute *as9947_36xkb_fan_attrs[] = {
    /* fan attributes */
    DECLARE_FAN_ATTR(1),
    DECLARE_FAN_ATTR(2),
    DECLARE_FAN_ATTR(3),
...
ATTRIBUTE_GROUPS(as9947_36xkb_fan);

static struct as9947_36xkb_fan_data *as9947_36xkb_fan_update_device(void)
{
    int status = 0;
                            ...
                            char *buf)
{
    ...
    data = as9947_36xkb_fan_update_device();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static struct as9947_36xkb_fan_data *as9947_36xkb_fan_update_cpld_ver(void)
{
    int status = 0;
...
static struct as9947_36xkb_fan_data *as9947_36xkb_fan_update_model_serial(int fan_id, int index)
{
    int status = 0;
    int string_size = 0 ;
                            ...
                            char *buf)
{
    ...
    mutex_lock(&data->update_lock);
    data = as9947_36xkb_fan_update_model_serial(fid, attr->index);
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
                                ...
                                char *buf)
{
    ...
    data = as9947_36xkb_fan_update_cpld_ver();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static int as9947_36xkb_fan_probe(struct platform_device *pdev)
{
    ...
    hwmon_dev = hwmon_device_register_with_info(&pdev->dev, DRVNAME,
                    NULL, NULL, as9947_36xkb_fan_groups);
...
static int __init as9947_36xkb_fan_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as9947_36xkb_fan_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as9947_36xkb_fan_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&as9947_36xkb_fan_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as9947_36xkb_fan_exit(void)
{
    if (data) {
...
MODULE_DESCRIPTION("as9947_36xkb_fan driver");
MODULE_LICENSE("GPL");

module_init(as9947_36xkb_fan_init);
module_exit(as9947_36xkb_fan_exit);
