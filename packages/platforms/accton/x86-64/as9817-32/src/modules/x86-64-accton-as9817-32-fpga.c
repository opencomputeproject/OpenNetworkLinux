...
#define OCORES_I2C_DRVNAME             "ocores-as9817-32"
                            ...
                            const char *buf, size_t count)
{
    ...
    u16 reg;
    u8 input;
    u8 reg_val = 0, bit_mask, should_set_bit;
    u32 spi_mask = 0;
...
static const struct _port_data port[PORT_NUM]= {
    ...
    {0x3520, SPI_BUSY_MASK_CPLD2},/* 0x3520 - 0x3530  CPLD2 I2C Master SFP-28 Port24 */
};
                                             ...
                                             unsigned int offset)
{
    ...
    if (!driver_find(OCORES_I2C_DRVNAME, &platform_bus_type)) {
        pcie_err("Platform driver %s not found, delaying device registration\n", OCORES_I2C_DRVNAME);
        err = -ENODEV;
        goto exit;
    }
