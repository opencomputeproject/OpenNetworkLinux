...
#define DRVNAME "as7946_30xb_led"
...
struct as7946_30xb_led_data *data = NULL;
...
enum as7946_30xb_led_sysfs_attrs {
	LED_LOC,
	LED_DIAG,
	LED_ALARM,
	LED_FAN,
...
static struct attribute *as7946_30xb_led_attributes[] = {
...
static const struct attribute_group as7946_30xb_led_group = {
	.attrs = as7946_30xb_led_attributes,
};
...
static struct as7946_30xb_led_data *as7946_30xb_led_update_device(void)
{
	int status = 0;
							...
							char *buf)
{
	...
	data = as7946_30xb_led_update_device();
	if (!data->valid) {
		error = -EIO;
		goto exit;
	}
...
static int as7946_30xb_led_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &as7946_30xb_led_group);
	if (status)
		goto exit;
...
static int __init as7946_30xb_led_init(void)
{
	...
	data = kzalloc(sizeof(struct as7946_30xb_led_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto alloc_err;
	}
	...
	ret = platform_driver_register(&as7946_30xb_led_driver);
	if (ret < 0)
		goto dri_reg_err;
...
dev_reg_err:
	platform_driver_unregister(&as7946_30xb_led_driver);
dri_reg_err:
	kfree(data);
alloc_err:
	return ret;
...
static void __exit as7946_30xb_led_exit(void)
{
...
MODULE_DESCRIPTION("as7946_30xb_led driver");
MODULE_LICENSE("GPL");

module_init(as7946_30xb_led_init);
module_exit(as7946_30xb_led_exit);