...
_onlp_fani_info_get_fan(int fid, onlp_fan_info_t* info)
{
    ...
    /* get front fan percentage
     */
    if (onlp_file_read_int(&value, "%s""fan%d_pwm", FAN_BOARD_PATH, fid) < 0) {
        AIM_LOG_ERROR("Unable to read status from (%s)\r\n", FAN_BOARD_PATH);
        return ONLP_STATUS_E_INTERNAL;
    }
    info->percentage = value;

    /* get rear fan percentage
     */
    if (onlp_file_read_int(&value, "%s""fan%d_pwm", FAN_BOARD_PATH, fid+CHASSIS_FAN_COUNT) < 0) {
        AIM_LOG_ERROR("Unable to read status from (%s)\r\n", FAN_BOARD_PATH);
        return ONLP_STATUS_E_INTERNAL;
    }
    /* take the min value from front/rear fan speed
     */
    if (info->percentage > value) {
        info->percentage = value;
    }
