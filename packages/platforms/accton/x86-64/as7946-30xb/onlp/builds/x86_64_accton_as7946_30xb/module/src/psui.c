...
{
    const char *ac_models[] = {"FSH082", "FSF019", NULL };
    int i;
...
onlp_psui_info_get(onlp_oid_t id, onlp_psu_info_t* info)
{
    ...
    *info = pinfo[pid]; /* Set the onlp_oid_hdr_t */
    ...
    val = 0;
    if (onlp_file_read_int(&val, "%s""psu%d_iin", PSU_SYSFS_PATH, pid) == 0 && val) {
        info->miin = val;
    ...
    val = 0;
    if (onlp_file_read_int(&val, "%s""psu%d_pin", PSU_SYSFS_PATH, pid) == 0 && val) {
        info->mpin = val;
