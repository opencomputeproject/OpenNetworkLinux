...
static ssize_t show_led(struct device *dev, struct device_attribute *attr,
            char *buf);
static int as7927_50x_led_probe(struct platform_device *pdev);
static int as7927_50x_led_remove(struct platform_device *pdev);

struct as7927_50x_led_data {
...
struct as7927_50x_led_data *data = NULL;

static struct platform_driver as7927_50x_led_driver = {
    .probe = as7927_50x_led_probe,
    .remove = as7927_50x_led_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
static const struct attribute_group as7927_50x_led_group = {
    .attrs = as7927_50x_led_attributes,
};

static struct as7927_50x_led_data *as7927_50x_led_update_device(void)
{
    int status = 0;
...
static int as7927_50x_led_probe(struct platform_device *pdev)
{
    ...
    status = sysfs_create_group(&pdev->dev.kobj, &as7927_50x_led_group);
    if (status)
        goto exit;
...
static int as7927_50x_led_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &as7927_50x_led_group);

    return 0;
}

static int __init as7927_50x_led_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as7927_50x_led_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as7927_50x_led_driver);
    if (ret < 0)
        goto dri_reg_err;
...
static void __exit as7927_50x_led_exit(void)
{
...
MODULE_DESCRIPTION("as7927_50x_led driver");
MODULE_LICENSE("GPL");

module_init(as7927_50x_led_init);
module_exit(as7927_50x_led_exit);
