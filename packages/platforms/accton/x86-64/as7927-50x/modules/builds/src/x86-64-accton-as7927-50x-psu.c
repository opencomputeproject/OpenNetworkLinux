...
static ssize_t show_string(struct device *dev, struct device_attribute *attr,
                            char *buf);
static int as7927_50x_psu_probe(struct platform_device *pdev);
static int as7927_50x_psu_remove(struct platform_device *pdev);
...
struct as7927_50x_psu_data *data = NULL;

static struct platform_driver as7927_50x_psu_driver = {
    .probe = as7927_50x_psu_probe,
    .remove = as7927_50x_psu_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
enum as7927_50x_psu_sysfs_attrs {
    /* psu attributes */
    PSU_ATTR(1),
    PSU_ATTR(2),
    NUM_OF_PSU_ATTR,
...
static struct attribute *as7927_50x_psu1_attrs[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(1),
...
static struct attribute_group as7927_50x_psu1_group = {
    .attrs = as7927_50x_psu1_attrs,
};
/* ATTRIBUTE_GROUPS(as7927_50x_psu1); */
...
static struct attribute *as7927_50x_psu2_attrs[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(2),
...
static struct attribute_group as7927_50x_psu2_group = {
    .attrs = as7927_50x_psu2_attrs,
};
/* ATTRIBUTE_GROUPS(as7927_50x_psu2); */

const struct attribute_group *as7927_50x_psu_groups[][2] = {
    {&as7927_50x_psu1_group, NULL},
    {&as7927_50x_psu2_group, NULL}
};

static struct as7927_50x_psu_data *as7927_50x_psu_update_device(struct device_attribute *da)
{
                            ...
                            char *buf)
{
    ...
    data = as7927_50x_psu_update_device(da);
    if (!data->valid[pid]) {
        error = -EIO;
        goto exit;
    }
...
static int as7927_50x_psu_probe(struct platform_device *pdev)
{
    ...
    for (i = 0; i < 2 ; i++) {
        hwmon_dev = hwmon_device_register_with_info(&pdev->dev, DRVNAME, 
                        NULL, NULL, as7927_50x_psu_groups[i]);
        if (IS_ERR(hwmon_dev)) {
...
static int __init as7927_50x_psu_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as7927_50x_psu_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as7927_50x_psu_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&as7927_50x_psu_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as7927_50x_psu_exit(void)
{
...
MODULE_DESCRIPTION("as7927_50x_psu driver");
MODULE_LICENSE("GPL");

module_init(as7927_50x_psu_init);
module_exit(as7927_50x_psu_exit);
