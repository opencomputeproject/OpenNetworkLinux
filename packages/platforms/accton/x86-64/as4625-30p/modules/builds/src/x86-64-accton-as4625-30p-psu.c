/*
 * An hwmon driver for accton as4625_30p Power Module
 *
...
struct as4625_30p_psu_data {
	...
	char serial[MAX_SERIAL_NUMBER]; /* Serial number, read from eeprom*/
	char fan_dir[FAN_DIR_LEN];
};

static struct as4625_30p_psu_data 
	*as4625_30p_psu_update_device(struct device *dev);

enum as4625_30p_psu_sysfs_attributes {
	PSU_PRESENT,
	PSU_MODEL_NAME,
	PSU_POWER_GOOD,
	PSU_SERIAL_NUMBER,
...
struct model_name_info models[] = {
	{ PSU_UP6001R_1079G,      0x20, 13, "UP6001R-1079G" },
};
...
static struct attribute *as4625_30p_psu_attributes[] = {
			 ...
			 char *buf)
{
	...
	struct as4625_30p_psu_data *data = i2c_get_clientdata(client);
	struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
	u8 status = 0;

	mutex_lock(&data->update_lock);

	data = as4625_30p_psu_update_device(dev);
	if (!data->valid) {
...
static const struct attribute_group as4625_30p_psu_group = {
	.attrs = as4625_30p_psu_attributes,
};
...
static int as4625_30p_psu_probe(struct i2c_client *client,
			const struct i2c_device_id *dev_id)
{
	struct as4625_30p_psu_data *data;
	int status;
	...
	data = kzalloc(sizeof(struct as4625_30p_psu_data), GFP_KERNEL);
	if (!data) {
		status = -ENOMEM;
		goto exit;
	}
	...
	status = sysfs_create_group(&client->dev.kobj, &as4625_30p_psu_group);
	if (status)
		goto exit_free;
...
exit_remove:
	sysfs_remove_group(&client->dev.kobj, &as4625_30p_psu_group);
exit_free:
	kfree(data);
exit:
	return status;
...
static int as4625_30p_psu_remove(struct i2c_client *client)
{
	struct as4625_30p_psu_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id as4625_30p_psu_id[] = {
	{ "as4625_30p_psu1", as4625_30p_psu1 },
	{ "as4625_30p_psu2", as4625_30p_psu2 },
	{}
};
MODULE_DEVICE_TABLE(i2c, as4625_30p_psu_id);

static struct i2c_driver as4625_30p_psu_driver = {
	.class		= I2C_CLASS_HWMON,
	.driver = {
		.name	 = DRVNAME,
	},
	.probe		= as4625_30p_psu_probe,
	.remove	   = as4625_30p_psu_remove,
	.id_table	 = as4625_30p_psu_id,
	.address_list = normal_i2c,
};

static int as4625_30p_psu_read_byte(struct i2c_client *client, u8 command,
					 u8 *data)
{
	int status = 0;
	int retry_count = 5;
	...
	if (unlikely(status < 0)) {
		dev_dbg(&client->dev, 
			"sfp read byte data failed, command(0x%2x), data(0x%2x)\r\n",
			 command, status);
		goto abort;
	}
...
static int as4625_30p_psu_read_bytes(struct i2c_client *client, u8 command, 
					u8 *data, int data_len)
{
	...
	while (data_len) {
		...
		status = as4625_30p_psu_read_byte(client, command, data);
		if (status <= 0) {
			ret = status;
			break;
		}
...
static struct as4625_30p_psu_data *as4625_30p_psu_update_device(
						struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct as4625_30p_psu_data *data = i2c_get_clientdata(client);
	...
	if (time_after(jiffies, data->last_updated + HZ + HZ / 2)
		...
		dev_dbg(&client->dev, "Starting as4625_30p update\n");
		data->valid = 0;
		...
		if (IS_POWER_GOOD(data->index, data->status)) {
			...
			/* Read model name */
			status = as4625_30p_psu_read_bytes(client, 
						models_min_offset, 
						temp_model_name,
						ARRAY_SIZE(temp_model_name));
			if (status < 0) {
				dev_dbg(&client->dev, 
					"unable to read model name from (0x%x)\n", 
					client->addr);
				goto exit;
			}
			...
			if (type < ARRAY_SIZE(serials)) {
				if ((serials[type].length+1) > ARRAY_SIZE(data->serial)) {
					dev_dbg(&client->dev,
						"invalid serials[%d].length(%d), should not exceed the size of data->serial(%ld)\n",
						type, serials[type].length, ARRAY_SIZE(data->serial));
					goto exit;
				}

				memset(data->serial, 0, sizeof(data->serial));
				status = as4625_30p_psu_read_bytes(client, serials[type].offset,
								data->serial,
								serials[type].length);
				if (status < 0) {
					dev_dbg(&client->dev,
...
module_i2c_driver(as4625_30p_psu_driver);

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@accton.com.tw>");
MODULE_DESCRIPTION("as4625_30p_psu driver");
MODULE_LICENSE("GPL");
