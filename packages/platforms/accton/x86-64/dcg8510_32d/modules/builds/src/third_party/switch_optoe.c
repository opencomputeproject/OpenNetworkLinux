...
#include <linux/version.h>
    ...
    #define i2c_new_dummy_device i2c_new_dummy
#endif
...
// For S3IP SONiC optoe access
#define MAX_SWITCH_OPTOE_PORT 32
struct optoe_data *s3ip_optoe_data[MAX_SWITCH_OPTOE_PORT]={NULL};
...
static const struct i2c_device_id optoe_ids[] = {
    { "switch_optoe1", ONE_ADDR },
    { "switch_optoe2", TWO_ADDR },
    { "switch_optoe3", CMIS_ADDR },
    { "switch_sff8436", ONE_ADDR },
    { "switch_24c04", TWO_ADDR },
    { /* END OF LIST */ }
};
...
EXPORT_SYMBOL_GPL(optoe_eeprom_read);

ssize_t optoe_eeprom_write(struct optoe_data *optoe,
                struct i2c_client *client,
                const char *buf,
                unsigned int offset, size_t count)
{
    ...
    default:
        ...
        msg.buf[i++] = offset;
#ifdef C11_ANNEX_K
        if(memcpy_s(&msg.buf[i], OPTOE_PAGE_SIZE+2, buf, count) != 0)
        {
            return -ENOMEM;
        }
#else
...
EXPORT_SYMBOL_GPL(optoe_eeprom_write);

ssize_t optoe_eeprom_update_client(struct optoe_data *optoe,
                char *buf, loff_t off,
...
EXPORT_SYMBOL_GPL(optoe_eeprom_update_client);
 ...
 */
ssize_t optoe_page_legal(struct optoe_data *optoe,
        loff_t off, size_t len, int eeprom_size)
{
...
EXPORT_SYMBOL_GPL(optoe_page_legal);

ssize_t optoe_read_write(struct optoe_data *optoe,
        char *buf, loff_t off, size_t len, int opcode, int eeprom_size)
{
    struct i2c_client *client = optoe->client[0];
    int chunk;
    int status = 0;
     ...
     */
    status = optoe_page_legal(optoe, off, len, eeprom_size);
...
EXPORT_SYMBOL_GPL(optoe_read_write);
        ...
        char *buf, loff_t off, size_t count)
{
    struct i2c_client *client = to_i2c_client(container_of(kobj,
                struct device, kobj));
    int eeprom_size = ONE_ADDR_EEPROM_SIZE;
    struct optoe_data *optoe = i2c_get_clientdata(client);

    return optoe_read_write(optoe, buf, off, count, OPTOE_READ_OP, eeprom_size);
}

int optoe_bin_eeprom_size_get(unsigned int index, int *eeprom_size)
{
    unsigned char Connector = 0, transmitter = 0;
    unsigned int conn_offset=0,transmitter_offset = 0,transmitter_bit = 0;
    ssize_t ret = 0;
    struct optoe_data *optoe = NULL;
    optoe = s3ip_optoe_data[index];
    if(optoe->dev_class == ONE_ADDR){
        conn_offset = QSFP_CONNECTOR_OFF;
        transmitter_offset = QSFP_TRANSMITTER_OFF;
        transmitter_bit = QSFP_TRANSMITTER_BIT;
    }else if (optoe->dev_class == CMIS_ADDR ){
        conn_offset = QSFP_DD_CONNECTOR_OFF;
        transmitter_offset = QSFP_DD_TRANSMITTER_OFF;
        transmitter_bit = QSFP_DD_TRANSMITTER_BIT;
    }
    ret = optoe_read_write(optoe, &Connector, conn_offset, sizeof(Connector), OPTOE_READ_OP,ONE_ADDR_EEPROM_SIZE);
    if(ret != sizeof(Connector)){
        dev_dbg(&optoe->client[0]->dev,
            "%s read :0x%x failed ret:%ld\n",
            __func__, conn_offset, ret);
        return ret;
    }else if(Connector == CONNECTOR_TYPE){
        ret = optoe_read_write(optoe, &transmitter, transmitter_offset, sizeof(transmitter), OPTOE_READ_OP,ONE_ADDR_EEPROM_SIZE);
        if(ret != sizeof(transmitter)){
            dev_dbg(&optoe->client[0]->dev,
                "%s read :0x%x failed ret:%ld\n",
                __func__, conn_offset, ret);
            return ret;
        }else if ((transmitter >> QSFP_TRANSMITTER_BIT & 0xf) >= TRANSMITTER_CROTICA_VALUE){
            *eeprom_size = ONE_ADDR_EEPROM_UNPAGED_SIZE;
        }else{
            if (optoe->dev_class == ONE_ADDR){
                *eeprom_size = ONE_ADDR_EEPROM_PAGED_SIZE;
            }else{
                *eeprom_size = CMIS_ADDR_EEPROM_SIZE;
            }
        }
    }else{
        if (optoe->dev_class == ONE_ADDR){
...
{
    int temp_optoe_index = 0;
    int port_num;
    int retval=0;
    int eeprom_size = ONE_ADDR_EEPROM_SIZE;
    for(temp_optoe_index=0; temp_optoe_index < MAX_SWITCH_OPTOE_PORT; temp_optoe_index++)
    {
        if(s3ip_optoe_data[temp_optoe_index] == NULL)
        {
            continue;
        }

        retval = kstrtoint(s3ip_optoe_data[temp_optoe_index]->port_name+sizeof(ETH_NAME_STRING)-1, 10, &port_num);
        if(retval != 0)
        {
            return -ENODEV;
        }
    ...
    if(temp_optoe_index == MAX_SWITCH_OPTOE_PORT)
    {
        return -ENODEV;
    }
    retval = optoe_bin_eeprom_size_get(temp_optoe_index,&eeprom_size);
    if(retval < 0){
        return retval;
    }
    return optoe_read_write(s3ip_optoe_data[temp_optoe_index], buf, off, len, OPTOE_READ_OP, eeprom_size);
}
EXPORT_SYMBOL_GPL(optoe_bin_read_by_index);
...
int optoe_bin_write_by_index(unsigned int index, char *buf, loff_t off, size_t len)
{
    ...
    return optoe_read_write(s3ip_optoe_data[temp_optoe_index], buf, off, len, OPTOE_WRITE_OP,eeprom_size);
}
EXPORT_SYMBOL_GPL(optoe_bin_write_by_index);
        ...
        char *buf, loff_t off, size_t count)
{
    ...
    return optoe_read_write(optoe, buf, off, count, OPTOE_WRITE_OP,eeprom_size);
}
...
static int optoe_remove(struct i2c_client *client)
{
    ...
    for(temp_optoe_index=0; temp_optoe_index < MAX_SWITCH_OPTOE_PORT; temp_optoe_index++)
    {
        ...
        if(s3ip_optoe_data[temp_optoe_index] == optoe)
        {
            s3ip_optoe_data[temp_optoe_index] = NULL;
        }
    }
            ...
            struct device_attribute *dattr, char *buf)
{
    ...
    mutex_lock(&optoe->lock);
#ifdef C11_ANNEX_K
    count = sprintf_s(buf, MAX_PORT_NAME_LEN, "%s\n", optoe->port_name);
#else
    count = sprintf(buf, "%s\n", optoe->port_name);
#endif
    mutex_unlock(&optoe->lock);
            ...
            const char *buf, size_t count)
{
...
#ifdef C11_ANNEX_K
    if (sscanf_s(buf, "%19s", port_name) < 0)
#else
    if (sscanf(buf, "%19s", port_name) < 0)
#endif
    {
        return -EINVAL;
    }
    ...
    mutex_lock(&optoe->lock);
#ifdef C11_ANNEX_K
    strcpy_s(optoe->port_name, MAX_PORT_NAME_LEN, buf);
#else
    strcpy(optoe->port_name, buf);
#endif
    mutex_unlock(&optoe->lock);
...
int optoe_probe(struct i2c_client *client,
            const struct i2c_device_id *id)
{
...
#ifdef EEPROM_CLASS
#ifdef C11_ANNEX_K
        if(strncpy_s(port_name, MAX_PORT_NAME_LEN, chip.eeprom_data->label, MAX_PORT_NAME_LEN) != 0)
        {
            return -ENOMEM;
        }
#else
...
#else
#ifdef C11_ANNEX_K
        if(memcpy_s(port_name, MAX_PORT_NAME_LEN, chip.port_name, MAX_PORT_NAME_LEN) != 0)
        {
            err = -ENOMEM;
            goto exit;
        }
...
#ifdef C11_ANNEX_K
        strcpy_s(port_name, MAX_PORT_NAME_LEN, "unitialized");
#else
...
#else
    /* Use the I2C_SMBUS_BYTE_DATA to avoid the incorrect data retrieving of some modules. */
    use_smbus = I2C_SMBUS_BYTE_DATA;
#endif
    ...
    if ((strcmp(client->name, "switch_optoe1") == 0) ||
        (strcmp(client->name, "switch_sff8436") == 0)) {
        /* one-address (eg QSFP) family */
        optoe->dev_class = ONE_ADDR;
        chip.byte_len = ONE_ADDR_EEPROM_SIZE;
        num_addresses = 1;
    } else if ((strcmp(client->name, "switch_optoe2") == 0) ||
           (strcmp(client->name, "switch_24c04") == 0)) {
        /* SFP family */
        optoe->dev_class = TWO_ADDR;
        chip.byte_len = TWO_ADDR_EEPROM_SIZE;
        num_addresses = 2;
    } else if (strcmp(client->name, "switch_optoe3") == 0) {
        /* CMIS spec */
...
#ifdef C11_ANNEX_K
    if(memcpy_s(optoe->port_name, MAX_PORT_NAME_LEN, port_name, MAX_PORT_NAME_LEN) != 0)
    {
        err = -ENOMEM;
        mutex_destroy(&optoe->lock);
        goto exit;
...
#if 0 /* Use the I2C_SMBUS_BYTE_DATA to avoid the incorrect data retrieving of some modules, not print notice messages here. */
    ...
    /* Give the optoe default port_name to ethxx, xx is i2c bus ID.
       bus 201 -> eth1, bus 202 -> eth2 ..., etc.
       KS SONiC tranciver use port name to map optoe. */
#ifdef C11_ANNEX_K
    if(sprintf_s(optoe->port_name, MAX_PORT_NAME_LEN, "%s%d", ETH_NAME_STRING, ((client->adapter->nr)%100)) < 0)
#else
    if(sprintf(optoe->port_name, "%s%d", ETH_NAME_STRING, ((client->adapter->nr)%100)) < 0)
#endif

    {
        dev_err(&client->dev, "switch_optoe sprintf failed\n");
        mutex_destroy(&optoe->lock);
        return -ENOMEM;
    }
    ...
    for(temp_optoe_index=0; temp_optoe_index < MAX_SWITCH_OPTOE_PORT; temp_optoe_index++)
    {
        ...
        s3ip_optoe_data[temp_optoe_index] = optoe;
        if(!s3ip_optoe_data[temp_optoe_index])
        {
            dev_err(&client->dev, "s3ip_optoe_data is NULL.\n");
            mutex_destroy(&optoe->lock);
            return -ENOMEM;
        }
        else
