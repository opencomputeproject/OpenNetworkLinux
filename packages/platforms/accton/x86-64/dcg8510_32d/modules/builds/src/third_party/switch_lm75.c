...
struct device *s3ip_hwmon_dev[TEMP_TOTAL_NUM] = {NULL};
...
int lm75_read_temp_max(unsigned int adapter_nr, long *val)
{
    struct lm75_data *s3ip_data; 
    struct i2c_client *s3ip_client;
    struct i2c_adapter *s3ip_adapter;
    int i;

    for(i=0; i<TEMP_TOTAL_NUM; i++)
    {
        if(s3ip_hwmon_dev[i] != NULL)
        {
            s3ip_data = dev_get_drvdata(s3ip_hwmon_dev[i]);
            s3ip_client = s3ip_data->client;
            s3ip_adapter = to_i2c_adapter(s3ip_client->dev.parent);
            
            if(s3ip_adapter->nr == adapter_nr)
                return lm75_read(s3ip_hwmon_dev[i], hwmon_temp, hwmon_temp_max, 0, val);
        }
    }

    return -1;
}
EXPORT_SYMBOL_GPL(lm75_read_temp_max); 

int lm75_read_temp_max_hyst(unsigned int adapter_nr, long *val)
{
    struct lm75_data *s3ip_data; 
    struct i2c_client *s3ip_client;
    struct i2c_adapter *s3ip_adapter;
    ...
    for(i=0; i<TEMP_TOTAL_NUM; i++)
    {
        if(s3ip_hwmon_dev[i] != NULL)
        {
            ...
            if(s3ip_adapter->nr == adapter_nr)         
                return lm75_read(s3ip_hwmon_dev[i], hwmon_temp, hwmon_temp_max_hyst, 0, val);
        }
    }

    return -1;
}
EXPORT_SYMBOL_GPL(lm75_read_temp_max_hyst); 

int lm75_read_temp_input(unsigned int adapter_nr, unsigned short chip_addr, long *val)
{
    struct lm75_data *s3ip_data; 
    struct i2c_client *s3ip_client;
    struct i2c_adapter *s3ip_adapter;
    ...
    for(i=0; i<TEMP_TOTAL_NUM; i++)
    {
        if(s3ip_hwmon_dev[i] != NULL)
        {
            ...
            if((s3ip_adapter->nr == adapter_nr)&&(s3ip_client->addr == chip_addr))
                return lm75_read(s3ip_hwmon_dev[i], hwmon_temp, hwmon_temp_input, 0, val);
        }
    }

    return -1;
}
EXPORT_SYMBOL_GPL(lm75_read_temp_input); 
