...
struct device *s3ip_hwmon_dev[TOTAL_VR_CHIP_NUM] = {NULL};
...
int pmbus_read_block_data(struct i2c_client *client, int page, int phase, u8 reg, unsigned char *data)
{
    int rv;
    ...
    return i2c_smbus_read_block_data(client, reg, data);
}
EXPORT_SYMBOL_GPL(pmbus_read_block_data);
...
int pmbus_core_read_attrs(unsigned int adapter_nr, unsigned short i2c_addr, char *attr_name, long *val)
{
    struct pmbus_data *s3ip_data = NULL;
    struct pmbus_sensor *s3ip_sensor;
    struct i2c_adapter *s3ip_adapter;
    struct i2c_client *s3ip_client;
    int i;

    for(i=0; i<TOTAL_VR_CHIP_NUM; i++)
    {
        if(s3ip_hwmon_dev[i] != NULL)
        {
            s3ip_client = to_i2c_client(s3ip_hwmon_dev[i]->parent);
            s3ip_adapter = to_i2c_adapter(s3ip_client->dev.parent);
            
            if((s3ip_adapter->nr == adapter_nr) && (s3ip_client->addr == i2c_addr))
            {
                s3ip_data = i2c_get_clientdata(s3ip_client);
                break;
            }
        }
    }
    ...
    for (s3ip_sensor = s3ip_data ->sensors; s3ip_sensor; s3ip_sensor = s3ip_sensor->next)
    {   
        if(!strcmp(s3ip_sensor->attribute.attr.name, attr_name))
            break;
    }

    if(s3ip_sensor->data < 0)
        return s3ip_sensor->data;

    *val = pmbus_reg2data(s3ip_data, s3ip_sensor);

    return 0;
}
EXPORT_SYMBOL_GPL(pmbus_core_read_attrs);

int pmbus_core_read_attrs_by_reg(unsigned int adapter_nr, unsigned short i2c_addr, int page, int reg, long *val)
{
    struct pmbus_data *s3ip_data = NULL;
    struct pmbus_sensor *s3ip_sensor;
    struct i2c_adapter *s3ip_adapter;
    ...
    for (s3ip_sensor = s3ip_data ->sensors; s3ip_sensor; s3ip_sensor = s3ip_sensor->next)
    {
        if((s3ip_sensor->reg == reg) && (s3ip_sensor->page == page))
            break;
    }
...
int pmbus_core_read_mfr_id(unsigned int adapter_nr, unsigned short i2c_addr, int *val)
{
    struct pmbus_data *s3ip_data = NULL;
    struct i2c_adapter *s3ip_adapter;
    struct i2c_client *s3ip_client;
    ...
    *val = _pmbus_read_word_data(s3ip_client, 0, 0xff, PMBUS_MFR_ID);

    return 0;
}
EXPORT_SYMBOL_GPL(pmbus_core_read_mfr_id);

int pmbus_core_read_reg(unsigned int adapter_nr, unsigned short i2c_addr, int page, int reg, PMBUS_PROTOCAL_TYPE_E type, void* val)
{
    struct pmbus_data *s3ip_data = NULL;
    struct i2c_adapter *s3ip_adapter;
    struct i2c_client *s3ip_client;
    ...
    switch(type)
    {
        case PMBUS_PROTOCAL_TYPE_WORD:
            *(int*)val = pmbus_read_word_data(s3ip_client, page, 0xff, reg);
            break;
        case PMBUS_PROTOCAL_TYPE_BYTE:
            *(int*)val = pmbus_read_byte_data(s3ip_client, page, reg);
            break;
        case PMBUS_PROTOCAL_TYPE_BLOCK:
            ret = pmbus_read_block_data(s3ip_client, page, 0xff, reg, (unsigned char*)val);
            if(ret < 0)
            {
                return -1;
            }
...
EXPORT_SYMBOL_GPL(pmbus_core_read_reg);

int pmbus_core_clear_fault(unsigned int adapter_nr, unsigned short i2c_addr)
{
    struct pmbus_data *s3ip_data = NULL;
    struct i2c_adapter *s3ip_adapter;
    struct i2c_client *s3ip_client;
    ...
    pmbus_clear_faults(s3ip_client);

    return 0;
}
EXPORT_SYMBOL_GPL(pmbus_core_clear_fault);
...
int pmbus_do_probe(struct i2c_client *client, struct pmbus_driver_info *info)
{
    ...
    for(i=0; i<TOTAL_VR_CHIP_NUM; i++)
    {
        if(s3ip_hwmon_dev[i] == NULL)
        {
            //Only record AVS device
            if(!strcmp(client->name, "mp2973") || !strcmp(client->name, "mp2940b") || !strcmp(client->name, "mp2976"))
            {
                s3ip_hwmon_dev[i] = data->hwmon_dev; 
                if(!s3ip_hwmon_dev[i])   
                {
...
int pmbus_do_remove(struct i2c_client *client)
{
    ...
    for(i=0; i<TOTAL_VR_CHIP_NUM; i++)
    {
        if(s3ip_hwmon_dev[i] != NULL)
        {
            ...
            if((s3ip_adapter->nr == adapter->nr) && (s3ip_client->addr == client->addr))
            {
                s3ip_hwmon_dev[i] = NULL;
                break;
            }
        }
    }
