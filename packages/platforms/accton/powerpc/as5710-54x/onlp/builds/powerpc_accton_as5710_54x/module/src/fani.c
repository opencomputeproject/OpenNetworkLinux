...
static last_path_T last_path[] =  /* must map with onlp_fan_id */
{
    ...
    MAKE_FAN_LAST_PATH_ON_PSU(5-003c),			
    MAKE_FAN_LAST_PATH_ON_PSU(6-003f),	
};
...
_onlp_fani_info_get_fan(int local_id, onlp_fan_info_t* info)
{
     ...
     */
    sprintf(fullpath, "%s%s", PREFIX_PATH_ON_MAIN_BROAD, last_path[local_id].status);	
    OPEN_READ_FILE(fd,fullpath,r_data,nbytes,len);
    if (atoi(r_data) > 0)
        info->status |= ONLP_FAN_STATUS_FAILED;

    sprintf(fullpath, "%s%s", PREFIX_PATH_ON_MAIN_BROAD, last_path[local_id].r_status);		
    OPEN_READ_FILE(fd,fullpath,r_data,nbytes,len);    
    if (atoi(r_data) > 0)
        info->status |= ONLP_FAN_STATUS_FAILED;
     ...
     */
    sprintf(fullpath, "%s%s", PREFIX_PATH_ON_MAIN_BROAD, last_path[local_id].speed);	
    OPEN_READ_FILE(fd,fullpath,r_data,nbytes,len);    
    info->rpm = atoi(r_data);

    sprintf(fullpath, "%s%s", PREFIX_PATH_ON_MAIN_BROAD, last_path[local_id].r_speed);	
    OPEN_READ_FILE(fd,fullpath,r_data,nbytes,len);    
...
{    
    ...
    switch (local_id)
	{
        ...
        case FAN_1_ON_PSU2:
            ...
            break;
        default:
            sprintf(fullpath, "%s%s", PREFIX_PATH_ON_MAIN_BROAD, last_path[local_id].ctrl_speed);
            break;
    }
