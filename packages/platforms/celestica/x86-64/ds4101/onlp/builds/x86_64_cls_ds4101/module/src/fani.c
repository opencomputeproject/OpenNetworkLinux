...
int onlp_fani_info_get(onlp_oid_t id, onlp_fan_info_t *info_p)
{
    ...
    *info_p = f_info[fan_id];
    ...
    if(fan_id <= CHASSIS_FAN_COUNT){
        get_fan_info(fan_id, info_p->model, info_p->serial, &airflow);
    }else{
        int psu_id = 0;
        ...
        if (!strcmp(info_p->model, PSU_F2B_AIRFLOW_FIELD)) {
            airflow = FAN_F2B;
        } else if (!strcmp(info_p->model, PSU_B2F_AIRFLOW_FIELD)) {
            airflow = FAN_B2F;
        }
    }

    spd_result = get_fan_speed(fan_id, &(info_p->percentage), &(info_p->rpm));
    if(spd_result || 0 == info_p->rpm){
