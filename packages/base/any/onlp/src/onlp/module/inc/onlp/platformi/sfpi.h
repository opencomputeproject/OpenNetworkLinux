 ...
 */
int onlp_sfpi_dev_read(int port, uint8_t devaddr, uint8_t addr, int size, uint8_t* rdata, uint32_t flags);

/**
 * @brief Write to an address on the given SFP port's bus.
 */
int onlp_sfpi_dev_write(int port, uint8_t devaddr, uint8_t addr, int size, uint8_t* data, uint32_t flags);
